...
> Last updated: 2026-07-03 (**Setup AI Validation Gates** — pre-AI / telemetry / post-AI validation safety layer for the Setup Brain. New pure-Python modules `data/setup_validation_result.py`, `data/setup_prompt_validation.py`, `data/setup_telemetry_validation.py`, `data/setup_output_validation.py`; `strategy/setup_ranges.resolve_effective_ranges`; three gates wired into `strategy/driving_advisor.build_combined_setup_response`; red/orange validation banners in `ui/setup_builder_ui.py`. New tests: `test_setup_ai_validation_gates.py` (**109 pass / 0 skip**, incl. the full Fuji/Panorama defect-case regression). No regressions in touched areas (329 pass across acceptance + setup-diagnosis + race-engineer). Commits `b7f1686` + `95eeb35`. See "Setup AI Validation Gates" at the end of this file.
...
## Setup AI Validation Gates (2026-07-03)

Professional validation/safety layer around the Setup Brain so the pipeline cannot show invalid, wrong-track, telemetry-corrupted, or driver-style-violating GT7 setups. Pipeline order: prompt → **pre-flight context validation** → AI → **schema + range + telemetry + driver-style + setup-logic validation** → only then show/save.

**New modules (pure-Python, no PyQt6, never raise):**
- `data/setup_validation_result.py` — `SetupValidationStatus`/`SetupValidationSeverity`/`RecommendedAction` enums, `SetupValidationIssue`, `SetupValidationResult` (`.blockers`/`.warnings`/`.to_dict()`), `make_validation_result`, `merge_results`. FAIL iff any BLOCKER; `safe_to_show_driver = status != FAIL`; `safe_to_apply_in_gt7 = no BLOCKER`.
- `data/setup_prompt_validation.py` — `validate_setup_prompt_context(event_ctx, track_location_id, layout_id, car_name, track_model_result, track_truth_result)`. Codes: `track_mismatch`, `layout_mismatch`, `car_mismatch` (BLOCKER); `track_model_not_ready` (BLOCKER for not_ai_ready/missing/error); `track_model_seed_only` (WARNING); `corner_geometry_suppressed` (INFO).
- `data/setup_telemetry_validation.py` — `assess_telemetry_sanity(laps)`, `build_telemetry_warning_block(result)`, `is_gearbox_corrupted/degraded`. `GEARBOX_CORRUPT_THRESHOLD=1.10`, `GEARBOX_DEGRADE_THRESHOLD=0.90`. Codes: `gearbox_corrupted`/`gearbox_degraded`/`gearbox_sanity_error`/`road_distance_negative` (all WARNING at this gate).
- `data/setup_output_validation.py` — `validate_setup_output(parsed_ai, effective_ranges, diagnosis, locked_fields, event_ctx, current_setup, car_name, telemetry_result)`. 30-field `_REQUIRED_OUTPUT_FIELDS`. Codes: `missing_output_field`, `field_out_of_range`, `driver_constraint_*`, `ride_height_without_proof`, `gearbox_changed_on_corrupt_telemetry` (BLOCKER); `gearbox_corrupt_preserved` (WARNING). Reuses `setup_diagnosis.validate_setup_engineering`.

**Wiring:** `strategy/setup_ranges.resolve_effective_ranges(car, event_ctx)` (generic < per-car < event `field_overrides`), shared by prompt + validator. `strategy/driving_advisor.build_combined_setup_response` runs the three gates (pre-AI short-circuit, telemetry warning injection, post-AI merge into `setup_validation`). `ui/setup_builder_ui._format_setup_validation_banner` + `_display_setup_result` render red FAIL / orange PASS_WITH_WARNINGS banners and hide Apply / skip history-save on FAIL.

**Locked decisions:** seed-only track → PASS_WITH_WARNINGS (not FAIL); corrupted gearbox telemetry forces gearbox preservation and only BLOCKS if the AI changed a gearbox field (does not hard-FAIL the whole setup).

**Tests:** `tests/test_setup_ai_validation_gates.py` — **109 pass / 0 skip**. Covers AC1–AC12: track/car mismatch, track-model confidence (incl. seed-only WARNING), gearbox corruption/degradation + threshold boundaries, negative road-distance, schema completeness, effective-range (incl. aero=0), driver hard-constraints, ride-height proof, gearbox consistency, the result schema + status derivation + merge, the UI banner (FAIL/WARN/PASS/escape/unknown), `resolve_effective_ranges` precedence, and the **full Fuji/Panorama defect-case regression** (FAIL with a BLOCKER for track mismatch + each out-of-range aero + ride-height-without-proof, and a `gearbox_corrupted` WARNING).

**Known limitations:** from-scratch `ai_planner.build_car_setup` path deferred (validators importable later); damper-logic & race-suitability only partial; end-to-end `build_combined_setup_response` not unit-tested (needs recorder/DB/API). Runtime UAT pending.
