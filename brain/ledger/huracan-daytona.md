...
2026-09-08,s145 -> s146,lsd_a,up (the 0.0089 threshold only - the direction is confirmed; next row),+12.0 (+6.0 from the pre-s145 setting),on-power rotation index T5 exit,T5 0.00104,config C (n=3); race config A (n=15),T5 index rises above 0.0089; front scrub below +0.0082,index at or below 0.00788,refuted,"§s145 'NEXT CHANGE, ISSUED, session 146'; §s146 'FALSIFIER 1 FAILED' (0.00748 n=10) - measured against the race baseline at a different rake",s146 (s147's table calls it config C)
2026-09-08,s145 -> s146,lsd_a,up,+12.0 (+6.0 from the pre-s145 setting),on-power rotation index T5 exit,T5 0.00104,the two lsd_a steps against each other,the T5 index moves with lsd_a across the two steps,no monotonic move clear of the floors,confirmed,"§s146 verdict: the axis is confirmed - monotonic at ~2x floors across the two steps (the threshold is the row above; split as D6 is)",s146 (s147's table calls it config C)
