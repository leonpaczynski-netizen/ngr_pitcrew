...
`verified = 1`"*, and its percentages are computed against that record. **The record exists:**
...
2026-09-06,none yet (run 2 on the sheet),df_r,down,step not stated in this file,none stated,not established,none,lowering df_r helps the front (the reason is in the car-state file),none stated,open,"§'What GT7's own readout says' last para ('That is run 2 on the sheet')",Rev A
2026-08-26,never run,lsd_a,up,+7.0,none stated,not established,none,none stated in this file (doctrine band cited at Rev A),none stated,open,"§'Open against this car' last row ('PROPOSED 26 Aug, NEVER RUN ... Demoted below the aero test'); Rev A DIFFERENTIAL block; step over the v1.71 record's span of 100, see (b)-S1",Rev A
...
- The `df_r` step size is not stated, so no delta can be computed. It is not in (a), because the
  file gives no absolutes for it either. The df_r span is 150 on the v1.71 range record
  (see (b)-S1), which the file's own arithmetic agrees with. Once a step is named, the delta
  will be step ÷ 150.
- **Not rows:** the driver's own `bb` trim is recorded and never corrected. The ballast
...
| a-S3 | Shelby × Deep Forest | `lsd_a` proposal of 26 Aug, never run | §"Open against this car", last row; Rev A DIFFERENTIAL block | it has one now (+7.0 pp against the v1.71 record, see (b)-S1); listed because the file still issues the differential in absolutes |
...
- **S1 (cross-file) - resolved 11 Sep.** The car-state header says "Range record: measured
  23 Aug 2026, GT7 v1.71, `verified = 1`". The range register (`11-car-slider-ranges.md`, updated
  24 Aug) lists the Shelby as "v1.70, STALE, do not issue … the only card left to turn". The first
  seeding read files only and left this open. **The database settles it:** `range_records` holds
  the Shelby at v1.71, verified, measured 23 Aug, and the car-state's ride-height, natural-frequency
  and downforce percentages all reproduce from it. Kept: the car-state file and the record agree;
  **the register is stale.** One consequence belongs to the register, not here: the record's LSD
  spans are 0-30 / 0-100 / 0-100, the same as the two Gr.3 cars, which is the result the register's
  "class-independence test" was waiting for.
