...
**The Lamborghini demands a freer diff than anyone tells you — and less caution than its reputation suggests.** The 10 Aug 2026 results reframed this car twice in one day. The diff finding: **the setting that makes it rotate is also the setting that makes the tyres last**, because less acceleration lock means less rear scrub. ⚠️ *[v1.70, and the rotation half is CONTESTED on v1.71 by this car's own measurement: "on this car LESS acceleration lock gives LESS rotation on power" (`brain/car-state/huracan-daytona.md`, s145) - `02` §10.5. The wear half is untouched by it.]* Where most cars force a trade between balance and stint length, the Huracán's accel axis moves both the same way — making it the first thing to set and the last thing to compromise.
