...
`HANDOFF-gearing-and-strategy-export.md` in the app repo.

This is a **change request against `10-pit-crew-data-format.md`**, kept separate on
purpose: doc 10 describes what the app actually emits today (`1.1`), and folding an
unbuilt schema into it would make a request read as a fact. **When the work lands and
a real 1.2 payload arrives, these sections move into doc 10 and this file is deleted.**
...
2. **Does 2nd cover all three chicanes without an upshift?** (Monza §3.1 — the
   constraint that outranks the final drive, and nothing verifies it)
3. **What is this car's real gearing constant K?** (Monza §3.5 — one reading makes
   every future gearbox on the car exact instead of iterated)

Plus: the strategy model computes a plan, makes live calls and stores an immutable
revision chain, and **`build_session_payload` has never once passed `strategy=`**. So
a returned setup has no idea what the strategy was or whether it held.

## What was already there — recorded on 11 Aug 2026

Checked against the working tree, and worth keeping in this knowledge base because it
corrects doc 10's and doc 12's "nothing built yet" framing:
...
Shape is unchanged from doc 10 §9, plus mandatory provenance on every assumption using
`strategy/evidence.py`'s existing vocabulary — `measured` / `declared` / `assumed` /
`missing`.
...
- **`wearMeasuredAtRaceMultiplier: false` must survive to the export.** Multiplier
  linearity remains assumed and never demonstrated — see the open items in
  `00-INDEX.md`.
