# Pit Crew Data — Ingestion Format

**Status, 11 Aug 2026:** the Pit Crew app is being rebuilt. This document has been
revised to **`gt7-pitcrew/1.1`** and now serves as **both** the ingestion spec for
this knowledge base and the export contract the app builds against. The v1.0 fields
that could not be produced honestly from GT7's actual UDP feed have been changed;
§14 lists every change and why.

The companion `12-pitcrew-app-brief.md` holds the app-side brief (what it does, the
transport facts, the strategy model). This document is the schema.

Leon pastes the payload below into the *Pit Crew data* box on the Driver Feedback tab
of the GT7 Race Engineering artifact. Until the export exists, that box stays empty
and diagnosis runs on the driver report alone.
...
Supersedes `gt7-pitcrew/1.0`. Changes and their justification are in §7.
...
Keys are the **shared vocabulary** (§6) — the same keys the consuming tool uses for
...
| `suspHeightMinMm` | Minimum **absolute** suspension height reached, per wheel. **Not travel remaining** — see §9.3. Interpret against `derived.bottomingRefMm` |
