...
| **kerb response** (the car, any axis) | — | ⚠️ **the car's second weakness — driver and instrument agree** 15 Sep | *"when the car hits a kerb it loses the planted feel"* (driver, after Bathurst Rd7). Upset share after big kerb strikes 0.48 Daytona / 0.42 Bathurst against **0.00 for the RSR** (measurement id 128; different car, hint not transfer). With Daytona held: `dc` softer **null**, `rh_r` down **null**, `arb_r` 4→6 **more rear slip after strikes, inside the floor**. Untested with a circuit held: `nf`, `de_f`, `rh_f`, ballast position. Monza run B tests `arb_r` softer (`brain/ledger/huracan-monza.md`) |
