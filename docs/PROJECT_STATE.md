...
> Last updated: 2026-07-03 (State Consolidation 2 — StrategyContext. New `data/strategy_context.py` (canonical, immutable strategy-plan read model owning stint plan / stops / fuel burn / `config_id` / degradation assumptions / tolerances; reads event/race rules from EventContext, never duplicates them) + `docs/STRATEGY_CONTEXT_MIGRATION.md`. `config["strategy"]` retained as legacy compatibility; one low-risk consumer (`_refresh_lap_bank` config_id ★ marker) migrated. **Full suite 4226 pass / 6 skip / 0 fail** (53 new tests). Prior: State Consolidation 1 — EventContext (`data/event_context.py`); Product Consolidation Sprint — audit + `ui/product_flow.py`)
...
| Metric | Value |
|--------|-------|
| Automated tests | 4226 passed / 6 skipped / 0 failed |
| Latest work | State Consolidation 2 — StrategyContext read model (`data/strategy_context.py`, `docs/STRATEGY_CONTEXT_MIGRATION.md`); one consumer migrated (`_refresh_lap_bank` config_id ★) — 53 new tests |
| Prior work | State Consolidation 1 — EventContext read model (`data/event_context.py`, `docs/EVENT_CONTEXT_MIGRATION.md`); one consumer migrated (`_refresh_telemetry_context`) — 38 new tests. Earlier: Product Consolidation Sprint — audit + safe UI clean-up — 27 new tests |
