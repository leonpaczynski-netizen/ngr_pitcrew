...
tests (J1–J9) green, 0 defects**, no Setup/Strategy Brain doctrine changed; gates #25–26
certified; **GO for controlled live activation**. The **6 pre-existing red tests (4 families)**
are proven on base `3c3446e` (NOT Program-3-caused). Full detail: `docs/PROGRAM3_STATE.md`
+ `docs/PROGRAM3_PHASE_J_CERTIFICATION.md`.
**Next = Phase K** (live GT7/PSVR2 on-hardware UAT + the 7 golden scenarios). Proposed
activation order: Practice → Qualifying → PTT → per-lap snapshots → broader strategy triggers.
_(The UI-rebuild notes below are superseded by the Program-3 spine.)_
...
**Commit 1 — P57-59 corrections + production audits (A-D):** exact P57-59 counts `00111b4..fd66f74` = **21 A / 6 M / 0 D = 27 files** where 6 M = **2 SOURCE + 4 DOCS** (`ngr_live_pit_wall.py` is ADDED not modified; the "3 source + 3 docs" wording was wrong). Audit B — real-tracker field map: car/track/fuel/freshness/map-match provided; layout resolved-can-be-limited; **applied-setup fingerprint = a LOCAL PROXY** (GT7 does not broadcast the setup) → caps setup attribution, not Practice pace; an EXACT match is possible by legitimate LOCAL COMPOSITION of the context digest. Audit C — at `fd66f74` the live modules are pure domain (no threads); the off-thread worker is Phase 60 (corrected the Phase-58 panel docstring). Audit D — production nav ownership: extend the existing `TAB_LIVE`, reuse the existing track-map moving-dot authority, return to `TAB_HOME`; runtime-file hashes recorded + left unstaged.

**Phase 60 (production Live tab):** `runtime_context_resolution.py` (composes the live context digest from resolved canonical local state; EXACT only when context confirmed AND the proxy applied-setup matches; always flags the proxy limitation) + `live_pit_wall_controller.py` (15-state `reduce_live_state`; opening Live never starts the activity; `UNVERIFIABLE`→`LIMITED_MATCH`; stale end→`TELEMETRY_LOST`, clean end→`BINDING_REQUIRED`; `activity_completed` always False) + `live_pit_wall_build.py` (`build_live_pit_wall_view` — DB-FREE per-packet composition). `ui/dashboard.py` `_build_tracker_runtime_snapshot`/`_refresh_live_pit_wall` (off-thread `MechanismAnnotationWorker`, stale guard by (event,activity)); `ui/live_ui.py` hosts `NgrLivePitWallPanel` as the FIRST Live-tab widget (post `/ui-ux-pro-max`). TAB_LIVE dispatch refreshes the pit wall.

**Phase 61 (complete event loop):** `driver_event_loop.py` (`decide_activity_launch` reuses assess_start_readiness — launch needs readiness+explicit confirm; `advance_event_loop` never skips binding/debrief, never reaches cumulative update without a confirmed outcome) + `discipline_workflow.py` (mode by activity type; Race needs a finalised/accepted strategy — advisory warning, never an autonomous block) + `binding_debrief_workflow.py` (explicit ranked binding — never auto-bind; cumulative update only after confirmed outcome + VALID/LIMITED; Command Centre refresh from canonical truth) + `live_restart_recovery.py` (restore selected event/activity + pending binding/debrief via programme_resume; never restored COMPLETED/started; `is_stale_snapshot` event-switch rule).

**Phase 62 (certification):** extended `event_programme_certification.py` → `PRODUCTION_CERTIFICATION_AREAS` (28), `production_event_certification()` = per-area (automated domain + offscreen; **live-GT7/visual/voice = NONE + required-next-evidence**), overall **NOT_TESTED** (bounded by live areas, differentiated not collapsed); `runtime_field_limitations()` (14 fields; proxy applied-setup blocks exact setup identity/attribution, not Practice pace). Caps retained; OPERATIONALLY_READY not awarded. Golden net found ONE controller defect (`UNVERIFIABLE`→generic LIVE) remediated to `LIMITED_MATCH`.

**Tests:** 89 new / 10 files (context+controller 17, live_worker 6, live_tab_integration 4, briefing_launch 8, discipline 7, binding_debrief 7, restart_eventswitch 6, certification 8, golden 17, safety 9). **Proof: unit/property-metamorphic/runtime-DB/STATIC-runtime-snapshot/offscreen-UI; deterministic replay + manual visual UAT + live GT7 UAT + physical voice UAT NOT run (headless).** 11 commits `954218b`..`<commit11>`. Apply gate + voice gate untouched; no new migration; NO new telemetry listener (reuses the pipeline); voice off by default. Runtime/app-state files untouched. Program 2 now Phases 12-62. Phase 63 NOT started.

---

## Prior Objective (2026-07-20) — Engineering Brain PROGRAM 2, Phases 57-59: Real GT7 Runtime, NGR Live Pit Wall & Event Certification — COMPLETE
...
**Tests:** 90 new / 9 files (adapter 16, cadence 6, runtime_authority 10, pit_wall 10, pit_wall_ui 5, integration 11, cert 7, golden 17, safety 8). **Proof: unit/property/runtime-DB/STATIC-runtime-snapshot/offscreen-UI; manual visual UAT + live GT7 UAT + physical voice UAT NOT run.** 10 commits `71fa8e6`..`fd66f74`; **P57-59 exact counts (git name-status 00111b4..fd66f74): 21 A / 6 M / 0 D = 27 files, +2129/-3** (added = 5 strategy + 2 UI + 9 tests + 5 docs; modified = 2 source [event_programme_certification.py, development_history_page.py] + 4 docs [MASTER_TESTING_REGISTER, CURRENT_CLAUDE_HANDOFF, PHASE56_OPERATIONAL_CERTIFICATION, PROJECT_STATE]; ngr_live_pit_wall.py is ADDED not modified — the mid-slice 16A/4M figure was superseded). Broad regression **4173 passed / 0 failed / 5873 deselected**. Apply gate + voice gate untouched; no new migration; NO new telemetry listener (reuses the pipeline). Program 2 now Phases 12-59. Phase 60 NOT started.
...
**Tests:** 112 new / 11 files (canonical_truth 21, truth_db 8, lock_strategy 10, next_action 7, bridge_match 9, bridge_views 8, session_end 9, certification 11, certification_ui 5, golden 16, safety 8 = 112). Query shape constant; truth+CC views byte-identical across refreshes. **Proof automated only (unit/property/runtime-DB/offscreen-UI + STATIC runtime-snapshot tests — NOT replay-timeline); manual visual UAT + live GT7 UAT NOT run.** 11 commits `724ba07`..`00111b4`; **P54-56 exact counts (git name-status da9d6db..00111b4): 24 A / 6 M / 0 D = 30 files, +2761/-10** (added = 6 strategy + 2 UI + 11 tests + 5 docs; the completion-report narrative's "9 strategy + 4 docs" was wrong). Apply gate + voice gate untouched. Program 2 now Phases 12-56. Phase 57 NOT started.
...
**Tests:** 117 new / 10 files (command_centre 17, command_centre_ui 6, dashboard_integration 7, live_activity 17, live_modes 6, binding_debrief 8, resume 11, revision_reopen_cert 17, golden 19, safety 9 = 117). Metamorphic: refresh-cannot-advance, selection-cannot-change-evidence, restart-cannot-complete, telemetry-cannot-strengthen, newest-cannot-autobind, invalid-cannot-update, noise-cannot-reopen, revision-cannot-rewrite, voice-cert-not-from-Home. Query shape constant (1/20 sessions); Command Centre view byte-identical across refreshes. **Proof was automated only (unit/property/runtime-DB/offscreen-UI); manual visual UAT + live GT7 UAT NOT run (headless); OperationalCertification = AUTOMATED_ONLY.** 10 commits `7af9e12`..`da9d6db`; **P51-53 exact counts (git name-status ef49d6c..da9d6db): 26 A / 15 M / 0 D = 41 files, +3489/-26** (corrected in the Phase 54-56 slice — the earlier 20A/11M/31/+2975 was a mid-slice measurement). Program 2 now Phases 12-53. Phase 54 NOT started.
...
**Phase 32 audit gate remediated (commit 1):** the Phase-32 fingerprint now covers EVERY material candidate field (full ordered candidate dicts via `content_fingerprint_for_candidates`; 13 mutation tests); (domain, investigation_type) grouping verified intentional (no cross-domain over-merge) and documented; exact git counts (Phase 32 = 11 added / 6 modified) verified + corrected in the Phase-32 doc.

**8 commits:** (1) Phase-32 audit remediation; (2) canonical serialization + Phase 33 export; (3) Phase 34 comparison; (4) Phase 35 package + loader/validator + writer; (5) read-only SessionDB orchestration; (6) UI; (7) tests + golden + runtime; (8) documentation. **Files (verified `git diff --name-status 0e88b8e..070e6ab`):** 24 added (9 strategy/1 data-writer/2 ui + 10 tests/1 test-helper + 3 phase docs) + 8 modified (data/session_db.py, ui/dashboard.py, ui/development_history_page.py, strategy/assurance_engineering_priority.py, docs/ENGINEERING_BRAIN_PHASE32_ASSURANCE_PRIORITY.md, docs/PROJECT_STATE.md, docs/CURRENT_CLAUDE_HANDOFF.md, MASTER_TESTING_REGISTER.md). **NEW pure** `strategy/assurance_chain_serialization.py` (the one canonical serializer), `strategy/assurance_chain_export.py` (+render), `strategy/assurance_snapshot_comparison.py` (+render), `strategy/assurance_manifest_loader.py` (strict baseline validator), `strategy/assurance_review_package.py` (+render). **NEW writer** `data/assurance_review_package_writer.py` (the ONLY file writer; explicit destination, staged/atomic/verified, no db/runtime/secrets, byte-deterministic zip). **NEW read-only SessionDB** `build_assurance_chain_export_report` / `build_assurance_snapshot_comparison_report` / `build_assurance_review_package_report` (+ `_assurance_chain_products`): chain built once, lower Phase 23-32 builders never called, baseline validation zero DB reads, constant query count, no N+1, Phase-29 gate unchanged. **UI** `AssuranceReviewPackPanel` (Preview/Compare Baseline.../Export Review Package... explicit actions; off-thread build+write; stale guards; NO Apply/experiment/campaign/schedule/editable controls; destination shown outside report). **Fingerprint hierarchy:** subordinate -> assurance-chain (over recomputed content digests; label-reuse-proof) -> canonical-manifest -> comparison (direction-material) -> review-package (over sorted (kind,digest); destination-free). **Doctrine:** newer!=better; no timestamp authority; domain-gone=incomparable not resolved; contradiction-closed-without-independence=unverified; assumption-dropped-without-evidence!=improvement; readiness-up-needs-independence; grade movement corroboration-gated; INCOMPATIBLE shows no trend; missing!=negative. **NEW suites (135 tests, all green):** `test_phase32_fingerprint[13]`, `test_phase33_export[14]`, `test_phase34_comparison[19]`, `test_phase35_package[20]`, `test_phase33_35_{query_shape[7],safety[34],golden[8],runtime[6],ui[14]}`. 12 golden fixtures; runtime-verified DB byte-identical, restart/shuffled-row byte-identical, package verifies independently + corruption fails, end-to-end export off the UI thread mutates no DB, explicit export is the only write path, no setup values/secrets/paths. Combined Phase 26-35 UI run 85 passed (intermittent shared-app.exec() artifact did not reproduce). Docs: `docs/ENGINEERING_BRAIN_PHASE33_ASSURANCE_EXPORT.md`, `..._PHASE34_ASSURANCE_COMPARISON.md`, `..._PHASE35_REVIEW_PACKAGE.md`. **An export/review package is NOT an independent certification. Program 2 now spans Phases 12-35. Recommended next (NOT started): a combined Phases 36-38 slice — 36 signed/attestable review manifest (deterministic detached signature over the package fingerprint, verify-only, no key generation in-app), 37 multi-snapshot assurance trend/timeline (>=3 compatible snapshots, still no timestamp authority), 38 reviewer annotation import (read-only ingestion of external reviewer notes as advisory overlay, never mutating knowledge).**

---

## Prior Objective (2026-07-20) — Engineering Brain PROGRAM 2, Phase 32: Assurance-Driven Engineering Priority — COMPLETE

**Branch `eng-brain-phase32-assurance-priority` from the Phase-31 tip `4b485be` — committed, NOT pushed / no PR / not merged (master does not contain Phase 32).** A deterministic, offline, READ-ONLY, ADVISORY-ONLY system that converts the Phase-31 assurance FINDINGS into a PRIORITISED list of EVIDENCE INVESTIGATIONS to improve programme assurance ("what evidence should the programme collect next, and why?"). Priority ORDER, never a schedule; no dates/sessions/drivers/cars/tracks/resources; no experiment/campaign/setup; no Apply; no AI/optimiser/scheduler; never guarantees a grade increase (impact POTENTIAL). Consumes Phase-17 info-gain DOCTRINE + Phase-22 chain + Phase 25–31 products; mutates none.

**NO migration / NO new persistence / NO DB write — `DB_VERSION` stays 26**, `RULE_ENGINE_VERSION` 46.0, `_setup_constants.py` byte-identical. NEW pure `strategy/assurance_engineering_priority.py` (`InvestigationPriorityBand` 6, `InvestigationType` 10, `InvestigationCandidate`, `build_investigation_candidates`, `build_assurance_engineering_priority`) — transparent scoring (raw·weight·contribution·rationale; info_gain 3.0 highest, mirrors Phase-17); doctrine: severity NOT sole rule (infeasible/dependent blocker → DEFER; cross-leverage can outrank single blocker), independent>dependent, contradictions need DISCRIMINATING evidence, assumptions stay assumptions, missing≠negative, confirmed-good protected, duplicates merge→leverage, deterministic prerequisites (independence→contradiction, version→context, context→convergence) DEFER dependents behind prereqs, NO scheduling; deterministic tie-break (band→score→blocker→leverage→info_gain→cost→canonical domain→type→id); timestamp-free fingerprint. + `strategy/assurance_engineering_priority_render.py`. NEW read-only `SessionDB.build_assurance_engineering_priority_report` (reuses `_build_knowledge_chain` ONCE; computes Phase-26/27/28/29/30/31 purely in memory; never calls lower SessionDB builders; no N+1; renderer zero DB; writes nothing; chain gate unchanged; negative-only analysable; empty truthful None). UI: `ui/assurance_engineering_priority_vm.py` + `ui/assurance_engineering_priority_panel.py` (`AssuranceEngineeringPriorityPanel`; NO Apply/Run/Create/Schedule/editable-priority controls; `[COLLECT]`/`[DEFER]` text tags) beneath the Phase-31 assurance verdict; OFF Qt thread via reused `MechanismAnnotationWorker` + stale-worker protection (`_priority_worker`). **NEW suites `tests/test_phase32_{domain[20],scoring[13],golden[9],integration[8],safety[14],ui_construction[11]}.py` (75)** — all green; golden = 8 mandated scenarios + shuffle-stability; integration proves Phase-22-once + nine lower SessionDB builders never called + constant query count (no N+1) + renderer-no-DB + DB-hash/counts/user_version unchanged + negative-only builds + empty truthful; safety proves no experiment/campaign/schedule/Apply/optimiser/scheduler/AI + no Phase-17 portfolio import + never-guarantees-grade + no setup-value/date leak. Phase 30/31 regression green. Runtime: DB byte-identical, restart + shuffled-legal-row-order identical rendering AND fingerprint, query count constant (7 vs 7), no setup-value/date leak, ASCII-clean, negative-only visible, empty truthful, fully-assured no-action. Known caveat: the UI off-thread test uses the shared-`QApplication`.`exec()` pattern (passes standalone; full 26–32 combined UI run: 71 passed — the intermittent multi-file timing artifact did not reproduce). Doc: `docs/ENGINEERING_BRAIN_PHASE32_ASSURANCE_PRIORITY.md`. **Program 2 now spans Phases 12–32. Recommended next: Phase 33 — deterministic read-only export / audit trail of the assurance chain 26→32 as an on-demand artefact (NOT started; no new persistence, no scheduling, no Apply).**

---

## Prior Objective (2026-07-19) — Engineering Brain PROGRAM 2, Phase 31: Knowledge Assurance & Audit Report (FINAL) — COMPLETE

**Branch `eng-brain-phase31-knowledge-assurance` from the Phase-30 tip `8265ea3` — committed, NOT pushed / no PR / not merged (master does not contain Phase 31).** The FINAL layer of Program 2: a deterministic, offline, READ-ONLY AUDIT of the whole knowledge programme (re-validation 26 + coverage 27 + readiness 28 + contradiction 29 + assumptions 30) for assurance FINDINGS, grading whether the knowledge can be ASSURED. Reports findings ONLY; authors no setup, changes nothing.

**Core doctrine (test-guarded):** a single BLOCKING finding PREVENTS ASSURED (→ NOT_ASSURED); defects = hidden assumptions, unresolved contradictions/regressions, missing transfer boundaries, single-context/dependent reliance, critical blind spots, stale/version-unaddressed, unknown-attribute/unverified-proxy reliance, readiness-vs-coverage + readiness-vs-assumption inconsistency, NON-DETERMINISM (a sub-report lacking a content fingerprint), DATA_MUTATION; grade RULE-BASED over visible severity counts (NOT opaque score), exposes counts+rule; too-little-knowledge → INSUFFICIENT_EVIDENCE; restart + shuffled rows + different legal order → byte-identical.

**NO migration / NO persistence — `DB_VERSION` stays 26**, `RULE_ENGINE_VERSION` 46.0. AssuranceFindingType 22; AssuranceSeverity 5; ProgrammeAssuranceGrade 5 (grade ladder no-known→INSUFFICIENT, any blocking→NOT_ASSURED, else major→PARTIALLY, else moderate/minor→WITH_LIMITATIONS, else→ASSURED). NEW pure `strategy/assurance_finding.py`, `strategy/assurance_grade.py`, `strategy/knowledge_assurance.py` (audit→(findings, has_known); dedup by (type,domain); structural fingerprint self-check; CLEAN when nothing above informational), `strategy/programme_assurance_report.py` (build_programme_assurance_report) + render. NEW entry `SessionDB.build_programme_assurance_report` (READ-ONLY; reuses shared `_build_knowledge_chain` [Phase-22 once + records]; computes Phase-26/27/28/29/30 PURELY in memory; never calls the Phase-23/24/25/26/27/28/29/30 DB entries; no N+1; renderer zero DB; writes nothing). UI: `ui/engineering_assurance_vm.py` + `ui/engineering_assurance_panel.py` (`EngineeringAssurancePanel`; NO Apply/edit/schedule controls; `[VERDICT]`/`[REVIEW]` text tags not colour alone) near the TOP of Development History beneath the Phase-28 readiness executive summary; OFF Qt thread via reused `MechanismAnnotationWorker` + stale-worker protection (`_assurance_worker`). **NEW suites `tests/test_phase31_{domain[16],golden[10],integration[8],safety[13],ui_construction[12]}.py` (59)** — all green; golden = mandated behaviours 1–10; integration proves Phase-22-once + Phase-23/24/25/26/27/28/29/30 DB entries never called + constant query count (no N+1) + renderer-no-DB + DB-hash/counts/user_version unchanged + no non-determinism/mutation finding from the real pipeline. Phase 26/27/28/29/30 regression green. `_setup_constants.py` git-verified byte-identical. Runtime: DB byte-identical, restart-identical fingerprint, user_version 26, no setup-value leak, ASCII-clean. Doc: `docs/ENGINEERING_BRAIN_PHASE31_KNOWLEDGE_ASSURANCE.md`. **Program 2 engineering-knowledge-assurance stack (Phases 12–31) COMPLETE. Phase 32 NOT started (authorisation stopped before 32). Recommended (not started): Phase 32 = assurance-driven engineering priority (rank what evidence to collect next to lift the grade); Phase 33 = deterministic read-only assurance-chain export / audit trail.**

---

## Prior Objective (2026-07-19) — Engineering Brain PROGRAM 2, Phase 30: Engineering Assumption Register — COMPLETE

**Branch `eng-brain-phase30-assumption-register` from the Phase-29 tip `2254996` — committed, NOT pushed / no PR / not merged (master does not contain Phase 30).** A deterministic, offline, READ-ONLY advisory layer that makes explicit the ASSUMPTIONS the current knowledge relies on but has NOT established (transfer, single-context generalisation, dependent-as-independent, currency, confirmed-good-persists, version-stability, contradiction-side, unknown-attribute, unverified-proxy). Classifies each + states impact. Lists ONLY assumptions, NEVER facts. Reuses Phase-24 boundaries + Phase-25/26/27/29 verbatim.

**Core doctrine (test-guarded):** FACTS ≠ ASSUMPTIONS (directly-evidenced conclusion — strong convergence + current + no gaps + no open contradiction — is a FACT → NO assumption; is_factual gates it); assumptions can ONLY CAP readiness, NEVER create it (AssumptionImpact has no positive member; IMPACT_READINESS_CAP never lifts above {not_ready, context_bound_only, ready_with_limitations, ready}); conservative bounds LABELLED; restart + shuffled rows + different legal order → byte-identical.

**NO migration / NO persistence — `DB_VERSION` stays 26**, `RULE_ENGINE_VERSION` 46.0. AssumptionType 16; AssumptionStatus 8; AssumptionImpact 6 (all non-positive). NEW pure `strategy/assumption_classification.py`, `strategy/assumption_impact.py`, `strategy/engineering_assumption.py` (is_factual + derive_domain_assumptions), `strategy/programme_assumption_register.py` (build_programme_assumption_register; per-domain + programme-level from Phase-24 boundaries; blocking/capping/narrowing/informational/conservative buckets) + render. NEW entry `SessionDB.build_programme_assumption_register` (READ-ONLY; reuses shared `_build_knowledge_chain` [Phase-22 once + records], computes Phase-26 re-validation + Phase-27 coverage + Phase-29 contradiction PURELY in memory; never calls the Phase-23/24/25/26/27/29 DB entries; no N+1; renderer zero DB; writes nothing). UI: `ui/engineering_assumption_vm.py` + `ui/engineering_assumption_panel.py` (`EngineeringAssumptionPanel`; NO Apply/edit/schedule controls; `[REVIEW]`/`[BOUND]` text tags not colour alone) in Development History beneath the Phase-29 panel; OFF Qt thread via reused `MechanismAnnotationWorker` + stale-worker protection (`_assumption_worker`). **NEW suites `tests/test_phase30_{domain[13],golden[10],integration[7],safety[13],ui_construction[12]}.py` (55)** — all green; golden = mandated behaviours 1–10; integration proves Phase-22-once + Phase-23/24/25/26/27/29 DB entries never called + constant query count (no N+1) + renderer-no-DB + DB-hash/counts/user_version unchanged. Phase 28/29 regression green. `_setup_constants.py` git-verified byte-identical. Runtime: DB byte-identical, restart-identical fingerprint, user_version 26, no setup-value leak, ASCII-clean. Doc: `docs/ENGINEERING_BRAIN_PHASE30_ASSUMPTION_REGISTER.md`. **Recommended next (FINAL): Phase 31 — knowledge assurance & audit report (not started); STOP before Phase 32.**

---

## Prior Objective (2026-07-19) — Engineering Brain PROGRAM 2, Phase 29: Knowledge Contradiction Resolution — COMPLETE

**Branch `eng-brain-phase29-contradiction-resolution` from the Phase-28 tip `c67607a` — committed, NOT pushed / no PR / not merged (master does not contain Phase 29).** A deterministic, offline, READ-ONLY advisory layer that finds, per known domain, where the evidence CONTRADICTS itself (a confirming AND a regressing conclusion for the same domain) and resolves each disagreement: by context, by stronger independent evidence, by supersession, or genuinely OPEN. Resolution STATUS ONLY; willing to say the evidence does not tell us which is right. Reuses the canonical Phase-25 `_record_domains` verbatim; grounds every contradiction in REAL records.

**Core doctrine (test-guarded):** NEVER by majority/averaging (count decides nothing; larger dependent count never wins); dependent NEVER defeats independent (independence = distinct sessions + high confidence, NOT count); newer does NOT auto-win (supersession needs later AND stronger, checked before independence); version/context mismatch SURFACED; a contradiction MAY stay UNRESOLVED; restart + shuffled rows + different legal order → byte-identical.

**NO migration / NO persistence — `DB_VERSION` stays 26**, `RULE_ENGINE_VERSION` 46.0. ContradictionCause 19; ContradictionStatus 9; ladder context→supersession(later&stronger)→independence→both-weak→insufficient→UNRESOLVED. NEW pure `strategy/contradiction_cause.py`, `strategy/contradiction_resolution_status.py` (`resolve`), `strategy/knowledge_contradiction.py` (`detect_contradiction`→`KnowledgeContradiction`), `strategy/programme_contradiction_report.py` (`build_programme_contradiction_report`; contradiction exists only where a domain has BOTH sides) + render. NEW entry `SessionDB.build_programme_contradiction_report`. **The shared `_build_knowledge_chain` gate was RELAXED** from "known_domains non-empty" to "known_domains OR any recorded evidence" (regressions retire domains out of known_domains, hiding contradiction evidence; the Phase-25 timeline already keeps negative learning visible → strict improvement; Phase 26/27/28 unaffected; empty programmes still yield no chain). No N+1; renderer zero DB; writes nothing. UI: `ui/engineering_contradiction_vm.py` + `ui/engineering_contradiction_panel.py` (`EngineeringContradictionPanel`; NO Apply/edit/schedule controls; `[OPEN]`/`[RESOLVED]` text tags not colour alone) in Development History beneath the Phase-27 panel; OFF Qt thread via reused `MechanismAnnotationWorker` + stale-worker protection (`_contradiction_worker`). **NEW suites `tests/test_phase29_{domain[15],golden[10],integration[6],safety[13],ui_construction[12]}.py` (56)** — all green; golden = mandated behaviours 1–10; integration proves Phase-22-once + Phase-23/24/25 DB entries never called + constant query count (no N+1) + renderer-no-DB + DB-hash/counts/user_version unchanged. Phase 25/26/27/28 regression (107) green. `_setup_constants.py` git-verified byte-identical. Runtime: DB byte-identical, restart-identical fingerprint, user_version 26, no setup-value leak, ASCII-clean. Doc: `docs/ENGINEERING_BRAIN_PHASE29_CONTRADICTION_RESOLUTION.md`. **Recommended next: Phase 30 — engineering assumption register (not started).**

---

## Prior Objective (2026-07-19) — Engineering Brain PROGRAM 2, Phase 28: Engineering Knowledge Readiness Report — COMPLETE

**Branch `eng-brain-phase28-knowledge-readiness` from the Phase-27 tip `d0a9ff7` — committed, NOT pushed / no PR / not merged (master does not contain Phase 28).** The EXECUTIVE-SUMMARY CAPSTONE of Program 2: a deterministic, offline, READ-ONLY advisory layer stating, per known domain, WHETHER the evidence supports RELYING on the knowledge for a decision (ready / ready within limits / provisional / not yet ready — and why), plus a TRANSPARENT RULE-BASED programme grade. Readiness STATUS ONLY: "ready" means the evidence supports relying on it, NEVER "apply this setup"; recommends no setup, schedules no test, creates no experiment/campaign; never marks unvalidated knowledge ready. Synthesises Phase-25 convergence + Phase-26 re-validation + Phase-27 coverage verbatim; invents NO domains.

**Core doctrine (test-guarded):** "ready" ≠ "apply this setup" (phrase only in denial text); unvalidated (converging/mixed/insufficient/unknown) is NEVER READY; a recorded conflict/regression BLOCKS readiness (CONFLICTED/REGRESSED) AND prevents a HIGH grade; grade is RULE-BASED over visible counts (NOT an opaque score/weight) and exposes counts + fired rule; too few assessable domains → INSUFFICIENT_EVIDENCE; restart + shuffled rows + different legal order → byte-identical (no timestamp in fingerprint).

**NO migration / NO persistence — `DB_VERSION` stays 26**, `RULE_ENGINE_VERSION` 46.0. KnowledgeReadinessStatus 11 (deterministic ladder; RELYABLE={ready,ready_with_limitations,context_bound_only}, BLOCKING={conflicted,regressed}); ProgrammeReadinessGrade 4 (HIGH/MEDIUM/LOW/INSUFFICIENT_EVIDENCE; MIN_ASSESSABLE=2, HIGH_RELYABLE_FRACTION=0.75, MEDIUM=0.40; any blocker caps below HIGH). NEW pure `strategy/knowledge_readiness.py` (`classify_readiness`→`KnowledgeReadinessItem`), `strategy/readiness_grade.py` (`grade_programme`→grade + counts + rule + reasons), `strategy/programme_readiness_report.py` (`build_programme_knowledge_readiness_report`; buckets + executive summary + grade detail) + `strategy/programme_readiness_report_render.py`. NEW entry `SessionDB.build_programme_knowledge_readiness_report` (READ-ONLY; reuses the shared `_build_knowledge_chain` [Phase-22 once + records], computes Phase-26 re-validation + Phase-27 coverage PURELY in memory; never calls the Phase-23/24/25/26/27 DB entries; no N+1; renderer zero DB; writes nothing). UI: `ui/engineering_readiness_vm.py` + `ui/engineering_readiness_panel.py` (`EngineeringReadinessPanel`; NO Apply/edit/schedule controls; `[READY]`/`[REVIEW]` text tags not colour alone; Phase 12/…/27 coexistence asserted) placed at the TOP of Development History (the executive summary); OFF Qt thread via reused `MechanismAnnotationWorker` + stale-worker protection (`_readiness_worker`). **NEW suites `tests/test_phase28_{domain[20],golden[10],integration[7],safety[13],ui_construction[12]}.py` (62)** — all green; golden = mandated behaviours 1–10; integration proves Phase-22-once + Phase-23/24/25/26/27 DB entries never called + constant query count (no N+1) + renderer-no-DB + DB-hash/counts/user_version unchanged. Phase 26/27 regression green. `_setup_constants.py` git-verified byte-identical. Runtime: DB byte-identical, restart-identical fingerprint, user_version 26, no setup-value leak, ASCII-clean. Doc: `docs/ENGINEERING_BRAIN_PHASE28_KNOWLEDGE_READINESS.md`. **Recommended next: Phase 29 — knowledge contradiction resolution (not started).**

---

## Prior Objective (2026-07-19) — Engineering Brain PROGRAM 2, Phase 27: Evidence Coverage & Blind-Spot Mapping — COMPLETE

**Branch `eng-brain-phase27-evidence-coverage` from the Phase-26 tip `23695ef` — committed, NOT pushed / no PR / not merged (master does not contain Phase 27).** A deterministic, offline, READ-ONLY advisory layer ABOVE the Phase-25 convergence authority: maps per known domain WHERE evidence is well supported and WHERE more evidence would strengthen confidence (a "blind spot"), across 18 visible COVERAGE DIMENSIONS, ranking blind spots by severity. Coverage STATUS ONLY: recommends no setup, schedules no test, creates no experiment/campaign, never treats absence of evidence as a negative result. Reuses Phase-23 transfer + Phase-25 convergence + Phase-26 re-validation verbatim; invents NO domains; reuses the canonical Phase-25 `_record_domains` mapping.

**Core doctrine (test-guarded):** MISSING (untested) is DISTINCT from REGRESSION_ONLY (a recorded negative) and never conflated — missing means untested, never wrong; a blind spot is NOT a fault (early-stage gaps → INFORMATIONAL); a large DEPENDENT count is never strong coverage; one distinct track/car/driver/compound/discipline/version is SINGLE_CONTEXT_ONLY; severity = reliance (maturity/confidence/confirmed-good) vs evidence robustness (independent lines/contexts/convergence) — strong claim on thin evidence → CRITICAL/MATERIAL, unresolved conflict in a relied-upon domain → CRITICAL; restart + shuffled rows + different legal order → byte-identical (no timestamp in fingerprint).

**NO migration / NO persistence — `DB_VERSION` stays 26**, `RULE_ENGINE_VERSION` 46.0. 18 dimensions = context-breadth (track/layout/car/driver/discipline/gt7_version/tyre_compound/corner_phase/corner_type) + evidence-quality (independent_replication/repeated_confirmation/high_confidence_evidence/regression_check/confirmed_good_verification/conflict_resolution/convergence_achieved/transfer_validation/revalidation_currency); CoverageStatus 9; BlindSpotSeverity 5. NEW pure `strategy/coverage_dimension.py`, `strategy/evidence_coverage.py` (`coverage_signals` + `assess_domain_coverage`→`DomainCoverage`), `strategy/knowledge_blind_spot.py` (`classify_blind_spot`→`KnowledgeBlindSpot`; reliance×robustness ladder; "not a fault … untested, never wrong"), `strategy/programme_coverage_report.py` (`build_programme_evidence_coverage_report`; buckets via reused canonical mapping) + `strategy/programme_coverage_report_render.py`. SHARED `SessionDB._build_knowledge_chain` MODIFIED to ADDITIVELY return the evidence records it read (single bulk read preserved) so Phase 27 derives breadth with NO 2nd query; NEW entry `SessionDB.build_programme_evidence_coverage_report` (READ-ONLY; Phase-26 re-validation computed purely in memory; never calls Phase-23/24/25/26 DB entries; no N+1; renderer zero DB; writes nothing). UI: `ui/engineering_coverage_vm.py` + `ui/engineering_coverage_panel.py` (`EngineeringCoveragePanel`; NO Apply/edit/schedule controls; `[REVIEW]`/`[COVERED]` text tags not colour alone; Phase 12/…/26 coexistence asserted) in Development History beneath the Phase-26 panel; OFF Qt thread via reused `MechanismAnnotationWorker` + stale-worker protection (`_coverage_worker`). **NEW suites `tests/test_phase27_{domain[17],golden[10],integration[7],safety[13],ui_construction[12]}.py` (69)** — all green; golden = mandated behaviours 1–10; integration proves Phase-22-once + Phase-23/24/25/26 DB entries never called + constant query count (no N+1) + renderer-no-DB + DB-hash/counts/user_version unchanged. Phase 25/26 regression green after the chain change. `_setup_constants.py` git-verified byte-identical. Runtime: DB byte-identical, restart-identical fingerprint, user_version 26, no setup-value leak, ASCII-clean. Doc: `docs/ENGINEERING_BRAIN_PHASE27_EVIDENCE_COVERAGE.md`. **Recommended next: Phase 28 — engineering knowledge readiness report (not started).**

---

## Prior Objective (2026-07-19) — Engineering Brain PROGRAM 2, Phase 26: Knowledge Decay & Re-validation Status — COMPLETE

**Branch `eng-brain-phase26-knowledge-revalidation` from the Phase-25 tip `30de0bb` — committed, NOT pushed / no PR / not merged (master does not contain Phase 26).** A deterministic, offline, READ-ONLY advisory layer ABOVE the Phase-25 convergence/timeline authority: reports per knowledge domain whether established knowledge stays CURRENT/PROTECTED or may need RE-VALIDATION — because context or GT7 version changed, or the evidence weakened (conflict / regression / dependence / unknown date-or-context). STATUS ONLY: schedules nothing, creates no reminders or future dates, generates no test plan, authors/applies no setup, never touches the Apply gate. Reuses Phase-25 verbatim.

**Core doctrine (test-guarded):** age ALONE never decays — NO date arithmetic anywhere in the pure layer (`timedelta`/`days_since`/`fromisoformat`/`strptime`/`max_age` absent + asserted; no fixed expiry); dates are evidence data never authority; newer never auto-more-correct; a version change re-validates ONLY version-sensitive domains (transfer-limitation mentions version), version-insensitive NOT invalidated; confirmed-good stays PROTECTED unless explicitly invalidated (unrelated regression does not un-protect); conflict weakens without deleting history; regression weakens/retires; superseded stays visible-but-inactive; retired stays retired; unknown date→INSUFFICIENT_DATE_EVIDENCE, unknown context→INSUFFICIENT_CONTEXT_EVIDENCE (never auto-invalidate); restart + shuffled rows + different legal order → byte-identical (no timestamp in fingerprint).

**NO migration / NO persistence — `DB_VERSION` stays 26**, `RULE_ENGINE_VERSION` 46.0. NEW pure `strategy/revalidation_reason.py` (`RevalidationReason` 20; `context_change_reason`; `reasons_from_signals` — reason emitted only when explicit signal present; GT7 reason gated on version_sensitive+version_changed), `strategy/knowledge_decay.py` (`programme_context_changes` — version_changed True when an excluded other-group shares source car+driver but differs in gt7_version; changed_fields from Phase-22 `excluded_reasons.differing_fields` VERBATIM; `decay_signals`; `MIN_INDEPENDENT_FOR_ROBUST`=2), `strategy/revalidation_status.py` (`KnowledgeFreshnessStatus` 12 + `FRESHNESS_PRIORITY` + `classify_revalidation` deterministic ladder), `strategy/programme_revalidation_report.py` (`build_revalidation_report(timeline, programme)`→`ProgrammeRevalidationReport`) + `strategy/programme_revalidation_report_render.py` (strings only, zero DB). NEW shared `SessionDB._build_knowledge_chain` (Phase-22 report EXACTLY ONCE, then Phase-23 transfer + Phase-24 playbook + Phase-25 timeline PURELY in memory via ONE bounded `_timeline_evidence_records` bulk read; never the Phase-23/24/25 DB entries) consumed by NEW entry `SessionDB.build_programme_revalidation_report` (READ-ONLY; no N+1; renderer zero DB; writes nothing). UI: `ui/engineering_revalidation_vm.py` + `ui/engineering_revalidation_panel.py` (`EngineeringRevalidationPanel`; NO Apply/Freeze/Complete/Execute/edit/schedule controls; `[PROTECT]`/`[REVIEW]` text tags not colour alone; Phase 12/19/…/25 coexistence asserted) in Development History beneath the Phase-25 panel; OFF Qt thread via reused `MechanismAnnotationWorker` + stale-worker protection (`_revalidation_worker`). **NEW suites `tests/test_phase26_{domain[20],golden[10],integration[7],safety[13],ui_construction[12]}.py` (70)** — all green; golden = mandated behaviours 1–10; integration proves Phase-22-once + Phase-23/24/25 DB entries never called + constant query count (no N+1) + renderer-no-DB + DB-hash/counts/user_version unchanged. `_setup_constants.py` git-verified byte-identical. Runtime: DB byte-identical after repeated runs, restart-identical fingerprint, user_version 26, no setup-value leak, ASCII-clean. Doc: `docs/ENGINEERING_BRAIN_PHASE26_KNOWLEDGE_REVALIDATION.md`. **Recommended next: Phase 27 — evidence coverage & blind-spot mapping (not started).**

---

## Prior Objective (2026-07-19) — Engineering Brain PROGRAM 2, Phase 25: Stable Knowledge Timeline & Convergence — COMPLETE

**Branch `eng-brain-phase25-knowledge-timeline` from the Phase-24 tip `8fae013` — committed, NOT pushed / no PR / not merged (master does not contain Phase 25).** A deterministic, offline, read-only TEMPORAL knowledge layer: how understanding evolved across compatible events, where evidence GENUINELY converged through INDEPENDENT repeated evidence, where it remains unresolved, and where apparent repetition is only duplicated/dependent. Dates are evidence data only — recency NEVER automatically means correctness. NEVER generates setup values, recommends changes, schedules tests, creates experiments/campaigns, optimises, applies, or mutates data.

**NO migration / NO persistence — `DB_VERSION` stays 26** (rebuilt from immutable records; restart- and shuffled-row-identical fingerprint; no timestamps), `RULE_ENGINE_VERSION` 46.0. NEW pure `strategy/evidence_independence.py` (`EvidenceIndependence` 7; repeated same-source evidence never = multiple confirmations; Phase-22/23/24 re-statements are one lineage), `strategy/knowledge_transition.py` (`KnowledgeTransitionType` 18; conflict/regression/supersession/uncertainty never collapsed; newer-weaker→NO_MATERIAL_CHANGE; stronger-later reopens retired), `strategy/knowledge_timeline.py` (`TimelinePoint` + `build_timeline`; sorts internally → insertion-order independent; unknown fields explicit), `strategy/knowledge_convergence.py` (`ConvergenceStatus` 10; strong needs ≥2 independent lines + established Phase-22 maturity + no unresolved regression/conflict; context-bound never strong; reuses Phase-22 maturity/confidence + Phase-24 confirmed-good), `strategy/programme_timeline_report.py` (joins Phase-22 graph + Phase-24 playbook + evidence records; maps records→domains via VISIBLE Phase-22 keyword maps; confirmation/regression counts from raw records so negative learning stays visible) + `strategy/programme_timeline_report_render.py`. Orchestrator `SessionDB.build_programme_knowledge_timeline` (READ-ONLY; Phase-22 report EXACTLY ONCE, Phase-23 + Phase-24 PURELY [never the Phase-23/24 DB entries], ONE bounded bulk read `_timeline_evidence_records` [query count constant]; no N+1; writes nothing). UI: `ui/engineering_timeline_vm.py` + `ui/engineering_timeline_panel.py` (`EngineeringTimelinePanel`; NO Apply/Create-Experiment/Schedule/Optimise/setup-editor/edit controls; confirmed-good `[PROTECT]`/conflict `[REVIEW]` text tags not colour alone; Phase 12/19/20/21/22/23/24 coexistence asserted) in Development History beneath the Phase-24 panel; OFF Qt thread via reused `MechanismAnnotationWorker` + stale-worker-result protection. **NEW suites `tests/test_phase25_{independence,convergence,timeline,integration,golden,safety,ui_construction}.py` (93)** — all green; Phase 17–24 non-UI regression (84) + session_db/no-AI (34) + Phase 22/23/24 UI (7+7+9) + Apply-gate/config-safety (125+1skip) green. `_setup_constants.py` + protected runtime files git-verified byte-identical. Runtime: independent convergence, dependent non-convergence, confirmed-good preservation, regression/retired, unresolved conflict, unknown dates, cross-car excluded; all table counts + DB-file SHA-256 unchanged, user_version 26, restart-identical fingerprint, no setup values. Doc: `docs/ENGINEERING_BRAIN_PHASE25_KNOWLEDGE_TIMELINE.md`. **Recommended next: Phase 26 — knowledge decay & re-validation prompts (not started).**

---

## Prior Objective (2026-07-19) — Engineering Brain PROGRAM 2, Phase 24: Cross-Programme Engineering Playbook — COMPLETE

**Branch `eng-brain-phase24-engineering-playbook` from the Phase-23 tip `4559bed` — committed, NOT pushed / no PR / not merged.** Assembles the reusable engineering knowledge across the driver's car stable into a deterministic read-only INVESTIGATION playbook (NOT a baseline setup): which mechanisms repeatedly matter, which confirmed-good behaviours to protect, which knowledge is safely reusable / a weak hypothesis / non-transferable, what to investigate first, what to recollect, and the explicit knowledge boundaries. NEVER generates/copies setup values, recommends a numerical starting setup, applies/schedules/optimises, creates experiments/campaigns, mutates/persists, calls AI, uses randomness/wall-clock, infers unknown vehicle attributes, creates a 2nd knowledge graph, or recreates Phase-23 transfer logic (TransferLevel reused verbatim).

**NO migration / NO persistence — `DB_VERSION` stays 26** (rebuilt from immutable records; restart-identical fingerprints; no timestamps in fingerprint), `RULE_ENGINE_VERSION` 46.0. NEW pure `strategy/stable_themes.py` (`StableEngineeringTheme` per established source domain; grounded not keyword), `strategy/investigation_priority.py` (6 categories via deterministic ladder + transparent weighted score over 11 VISIBLE dimensions/weights/caps; masking_conflict flag), `strategy/knowledge_boundary.py` (`BoundaryType` 16 incl unknown_vehicle_attribute [never guessed] + unverified_transfer_proxy), `strategy/new_programme_brief.py` (`NewProgrammeBrief` per target + explicit no-setup-transferred statement), `strategy/engineering_playbook.py` (`build_engineering_playbook`; joins Phase-22 graph + Phase-23 candidates, 4 layers, deterministic tie-breakers) + `strategy/engineering_playbook_render.py`. Orchestrator `SessionDB.build_programme_engineering_playbook` (READ-ONLY; composes Phase-22 report EXACTLY ONCE, derives Phase-23 transfer PURELY [never calls the Phase-23 DB entry, no 2nd Phase-22 build], no N+1; writes nothing; honest empty state). UI: `ui/engineering_playbook_vm.py` + `ui/engineering_playbook_panel.py` (`EngineeringPlaybookPanel`; NO Apply/Create-Experiment/Schedule/Optimise/setup-editor/import/copy-setup controls; confirmed-good visually distinct; Phase 12/19/20/21/22/23 coexistence asserted) in Development History beneath the Phase-23 panel; OFF Qt thread via reused `MechanismAnnotationWorker` with stale-worker-result protection. **NEW suites `tests/test_phase24_{domain,transfer_integration,query_shape,golden,safety,ui_construction}.py` (76)** — all green; Phase 17–23 non-UI regression (57) + session_db/no-AI (34) + Phase 22/23 UI (14) + config-safety (30) + Apply-gate (95+1skip) green. `_setup_constants.py` + protected runtime files git-verified byte-identical. Runtime: multi-car themes, related Porsche reusable, Toyota isolated, confirmed-good present; all table counts + DB-file sha256 unchanged; user_version 26; restart-identical fingerprint; no setup values. Doc: `docs/ENGINEERING_BRAIN_PHASE24_ENGINEERING_PLAYBOOK.md`. **Recommended next: Phase 25 — stable knowledge timeline & convergence (not started).**

---

## Prior Objective (2026-07-19) — Engineering Brain PROGRAM 2, Phase 23: Knowledge Transfer Eligibility & Cross-Car Engineering Reuse — COMPLETE

**Branch `eng-brain-phase23-knowledge-transfer` from the Phase-22 tip `0ce6721` — committed, NOT pushed / no PR / not merged.** A deterministic Engineering Knowledge TRANSFER layer — NOT setup transfer; it determines whether established engineering KNOWLEDGE (mechanisms, handling behaviour) is likely reusable in another compatible context (e.g. another Porsche Gr.3). Transfers NO setup values, recommends applying NOTHING, imports NOTHING, decides NOTHING; every transfer level fixed by VISIBLE deterministic rules (no inference beyond them). No ML/stats/NLP/AI/optimisation/scheduling/graph-libraries.

**NO migration / NO persistence — `DB_VERSION` stays 26** (all reconstructed from immutable records; restart-identical fingerprints), `RULE_ENGINE_VERSION` 46.0. NEW pure `strategy/transfer_rules.py` (all rules VISIBLE CONSTANTS each with why+authority: same_manufacturer/same_drivetrain/same_layout/same_race_category/same_suspension_architecture[mfr+category proxy]/compatible_gt7_version/same_driver; `car_attributes` derives manufacturer/drivetrain/layout/category from the GT7 car name via visible maps, unknown STAYS unknown; `DOMAIN_TRANSFER_CLASS` 17 domains → architecture_dependent/handling_drivetrain/car_track_specific/context_bound/driver_specific), `strategy/knowledge_transfer.py` (`TransferLevel` 6: NOT_TRANSFERABLE/VERY_LOW/LOW/MEDIUM/HIGH/SUPPORTED + `evaluate_transfer`; level decided ONLY by rules + domain class; established-source gate; gearbox car-track-specific only-if-explicitly-supported; context-bound/driver-specific/version handling; every field reason+source+calculation), `strategy/engineering_reuse.py` (`summarise_reuse` reusable/needs_more_evidence/not_reusable + isolated targets; NEVER recommends applying), `strategy/programme_transfer_report.py` (`build_transfer_report` every established source-domain × target + reuse summary + visible rule catalogue) + `strategy/programme_transfer_report_render.py`. Orchestrator `SessionDB.build_programme_transfer_report` (READ-ONLY; composes Phase-22 programme knowledge report ONCE [source domains + other groups as targets]; no per-campaign query; writes nothing). UI: `ui/engineering_transfer_vm.py` + `ui/engineering_transfer_panel.py` (`EngineeringTransferPanel`; NO Apply/Execute/Import/Copy-Setup/edit controls; Phase 12/19/20/21/22 coexistence asserted) in Development History beneath the Phase-22 panel; build OFF the Qt thread via the reused `MechanismAnnotationWorker`. **NEW suites `tests/test_phase23_{transfer_rules,eligibility,golden,query_shape,safety,ui_construction}.py` (62)** — all green; Phase 20–22 non-UI regression (incl. touched session_db/dashboard/development_history_page) green; no-AI architecture guard green. Golden `config_id` + frozen allowlist + Apply-gate + engine-wiring-status UNCHANGED; protected runtime files git-verified byte-identical (including setup_knowledge_base.py — Phase 23 keeps its own visible maps, imports nothing from it). Doc: `docs/ENGINEERING_BRAIN_PHASE23_KNOWLEDGE_TRANSFER.md`. **Recommended next: Phase 24 — cross-programme engineering playbook (not started).**

---

## Prior Objective (2026-07-19) — Engineering Brain PROGRAM 2, Phase 22: Engineering Knowledge Graph & Multi-Event Knowledge Roll-Up — COMPLETE

**Branch `eng-brain-phase22-knowledge-graph` from the Phase-21 tip `30df17e` — committed, NOT pushed / no PR / not merged.** The first programme-level Engineering Knowledge Graph: a deterministic, explainable map of what the Brain knows, how well, on what evidence, and what remains unknown — organised by ENGINEERING DOMAIN and rolled up across compatible events. NOT AI/graph-theory/ML/optimisation/scheduling — deterministic aggregation of existing authorities. NEVER decides/optimises/prioritises/schedules/completes/applies/creates/mutates. No ML/stats/NLP/AI/Bayesian/graph-libraries/network.

**NO migration / NO persistence — `DB_VERSION` stays 26** (all reconstructed from immutable records; restart-identical fingerprints), `RULE_ENGINE_VERSION` 46.0. NEW pure `strategy/engineering_knowledge_graph.py` (`KnowledgeDomain` 17 + `build_knowledge_graph`; VISIBLE keyword maps FIELD/FAMILY/MECHANISM_DOMAIN_KEYWORDS, unmapped→no domain NO inference; each DomainKnowledge = state/confidence/maturity/remaining_uncertainty[reason+source+calculation] + supporting_campaigns/experiments/mechanisms/evidence + known_limitations; enumerates ALL domains, empty→MISSING), `strategy/multi_event_rollup.py` (`build_rollup`; compatibility key = car+discipline+gt7_version+driver [track/layout may differ]; NEVER merges unlike contexts, every exclusion names differing fields), `strategy/knowledge_maturity.py` (`KnowledgeMaturity` 7: UNKNOWN/EMERGING/DEVELOPING/ESTABLISHED/MATURE/COMPLETE/PLATEAUED + `classify_maturity`, ladder over Phase-19/20/21, no invented weighting), `strategy/programme_knowledge_report.py` (`build_programme_knowledge` rolls up + builds graph for primary group, lists other groups + exclusions) + `strategy/programme_knowledge_report_render.py`. Orchestrator `SessionDB.build_programme_knowledge_report` (READ-ONLY; one SELECT DISTINCT over dev-records → distinct events; builds Phase-21 season report ONCE per COMPATIBLE event + enriches campaigns with Phase-21 knowledge_state [join not recompute]; incompatible events surfaced not merged; writes nothing). UI: `ui/engineering_knowledge_graph_vm.py` + `ui/engineering_knowledge_graph_panel.py` (`EngineeringKnowledgeGraphPanel`; NO Apply/edit/schedule controls; Phase 12/19/20/21 coexistence asserted) in Development History beneath the Phase-21 panel; build OFF the Qt thread via the reused `MechanismAnnotationWorker`. **NEW suites `tests/test_phase22_{maturity,knowledge_graph,rollup,golden,query_shape,safety,ui_construction}.py` (71)** — all green; Phase 19–21 non-UI regression (incl. touched session_db/dashboard/development_history_page) green; no-AI architecture guard green. Golden `config_id` + frozen allowlist + Apply-gate + engine-wiring-status UNCHANGED; protected runtime files git-verified byte-identical. Doc: `docs/ENGINEERING_BRAIN_PHASE22_ENGINEERING_KNOWLEDGE_GRAPH.md`. **Recommended next: Phase 23 — knowledge transfer & cross-car generalisation (not started).**

---

## Prior Objective (2026-07-19) — Engineering Brain PROGRAM 2, Phase 21: Season Development Plan & Cross-Campaign Knowledge Map — COMPLETE

**Branch `eng-brain-phase21-season-development` from the Phase-20 tip `dd0f1ea` — committed, NOT pushed / no PR / not merged.** A deterministic, READ-ONLY season-level planning layer (the Engineering Director's dashboard) that understands the programme AS A WHOLE: what has been learned, where the largest knowledge gaps are, which vehicle systems are understood, and how campaigns relate (overlap/duplicate/support/contradict/depend/isolated). It ONLY EXPLAINS engineering state — NEVER decides/optimises/reprioritises/schedules/completes/applies/creates. No ML/stats/NLP/AI/Bayesian/graph-optimisation/clustering/scheduling.

**NO migration / NO persistence — `DB_VERSION` stays 26** (all reconstructed from existing records; restart-identical fingerprints), `RULE_ENGINE_VERSION` 46.0. NEW pure `strategy/season_development.py` (`summarize_season` → `SeasonDevelopment`; each metric value+reason+source+calculation; totals reuse Phase-17 value + Phase-19 cost verbatim), `strategy/cross_campaign_map.py` (`CampaignRelationship` 9 + `build_cross_campaign_map`; deterministic O(n²) pairwise; every edge EVIDENCE-GROUNDED with reason+supporting_evidence+authority, never inferred; DUPLICATES/CONTRADICTS/DEPENDS_ON/SUPPORTS[shared Phase-18 source_mechanism + confidence asymmetry, directional]/OVERLAPS/BLOCKED_BY/RELATED/ISOLATED; NOT graph search/clustering/ML), `strategy/season_knowledge_map.py` (`SeasonKnowledgeState` 9 + `classify_campaign_knowledge`; ladder over Phase-18 status + Phase-19 saturation + Phase-20 confidence/opportunity; reason+source+factors), `strategy/season_engineering_report.py` (`build_season_report` joins Phase-18 programme + Phase-19 efficiency + Phase-20 quality by campaign_id into normalised records, PRESERVES order, runs 3 layers) + `strategy/season_engineering_report_render.py`. Orchestrator `SessionDB.build_season_engineering_report` (READ-ONLY; composes Phase-18 programme ONCE, derives Phase-19 efficiency + Phase-20 quality PURELY + 1 registry + 1 calibration read; no N+1, no double programme build; writes nothing). UI: `ui/engineering_season_vm.py` + `ui/engineering_season_panel.py` (`EngineeringSeasonPanel`; NO Apply/edit/schedule controls; Phase 12/19/20 coexistence asserted) in Development History beneath the Phase-20 panel; build OFF the Qt thread via the reused `MechanismAnnotationWorker`. **NEW suites `tests/test_phase21_{knowledge_map,relationships,development,golden,query_shape,safety,ui_construction}.py` (72)** — all green; Phase 18–20 non-UI regression (incl. touched session_db/dashboard/development_history_page) green; no-AI architecture guard green. Golden `config_id` + frozen allowlist + Apply-gate + engine-wiring-status UNCHANGED; protected runtime files git-verified byte-identical. Doc: `docs/ENGINEERING_BRAIN_PHASE21_SEASON_DEVELOPMENT.md`. **Recommended next: Phase 22 — multi-event season roll-up & engineering narrative (not started).**

---

## Prior Objective (2026-07-19) — Engineering Brain PROGRAM 2, Phase 20: Confidence-Weighted Evidence & Development ROI — COMPLETE

**Branch `eng-brain-phase20-confidence-and-roi` from the Phase-19 tip `f95345e` — committed, NOT pushed / no PR / not merged.** Three deterministic, ADVISORY-ONLY analysis layers built ABOVE Phase 19 that MEASURE and decide nothing: **knowledge confidence** (how trustworthy are the conclusions?), **development ROI** (how much engineering knowledge could a further session add, at what cost/risk?), and **campaign opportunity** (is more work worthwhile?). NEVER completes/freezes/abandons a campaign, applies/approves a setup, creates experiments, alters outcomes, re-ranks, recomputes Phase-17 value or Phase-19 cost, or auto-prioritises. No ML/stats/NLP/AI/optimiser/Bayesian/network.

**NO migration / NO persistence — `DB_VERSION` stays 26** (all reconstructed from existing records; restart-identical fingerprints), `RULE_ENGINE_VERSION` 46.0. NEW pure `strategy/knowledge_confidence.py` (`ConfidenceLevel` 6 + `assess_campaign_confidence`; 7 components each with reason/source/**calculation**; overall = equal-weighted mean of INCLUDED components banded by named constants + VISIBLE caps [no-evidence→UNKNOWN; conflicting→LOW; regression+0-confirm→VERY_LOW; <MIN_REPEATABILITY→MEDIUM mirroring Phase-18 VALIDATION_REQUIRED]; prediction_accuracy is context-level from Phase-11 calibration, excluded when uncalibrated; remaining_uncertainty informational). NEW pure `strategy/development_roi.py` (`estimate_campaign_roi`; engineering knowledge NOT lap time; info gain from Phase-19 + cost from Phase-19 verbatim; expected_confidence_gain full-for-discriminating/half-for-validation/0-if-untestable; engineering_priority_reason is an EXPLANATION not a ranking; NOT an optimiser — sorts/ranks nothing). NEW pure `strategy/campaign_opportunity.py` (`classify_campaign_opportunity` ladder → 8 outcomes + UNKNOWN; reads Phase-18 completion, NEVER overrides it). NEW pure `strategy/knowledge_quality.py` (`build_knowledge_quality` composes the three per campaign, PRESERVES Phase-19 order) + `strategy/engineering_knowledge_quality_render.py`. Orchestrator `SessionDB.build_engineering_knowledge_quality` (READ-ONLY; reuses Phase-19 `build_engineering_efficiency` ONCE [read-only, no registry write] + one calibration read; no N+1). UI: `ui/engineering_confidence_vm.py` + `ui/engineering_confidence_panel.py` (`EngineeringConfidencePanel` — deliberately NOT `EngineeringKnowledgePanel`, which Phase 12 owns; NO Apply/edit controls; Phase-12 panel coexistence asserted) in Development History beneath the Phase-19 panel; build OFF the Qt thread via the reused `MechanismAnnotationWorker`. **NEW suites `tests/test_phase20_{confidence,roi,opportunity,golden,query_shape,safety,ui_construction}.py` (74)** — all green; Phase 12–19 non-UI regression (incl. touched session_db/dashboard/development_history_page) green; no-AI architecture guard green. Golden `config_id` + frozen allowlist + Apply-gate + engine-wiring-status UNCHANGED; protected runtime files git-verified byte-identical. Doc: `docs/ENGINEERING_BRAIN_PHASE20_CONFIDENCE_AND_ROI.md`. **Recommended next: Phase 21 — season development plan & cross-campaign knowledge map (not started).**

---

## Prior Objective (2026-07-19) — Engineering Brain PROGRAM 2, Phase 19: Campaign Persistence, Evidence Saturation & Cost of Knowledge — COMPLETE

**Branch `eng-brain-phase19-campaign-persistence` from the Phase-18 tip — committed, NOT pushed / no PR / not merged.** Three deterministic, ADVISORY-ONLY layers built ABOVE Phase 18 that MEASURE (never decide): a campaign's **age** (from a new additive registry), its **evidence saturation** ("is more testing still worth it?"), and its **cost of knowledge** (estimated laps/tyres/fuel/time of the remaining testing, weighed against a session budget). Saturation is **independent of campaign status**; **completion stays governed by Phase 18**; the frozen Apply gate stays the sole route to the car. NEVER completes/freezes/abandons a campaign, applies/approves a setup, creates/updates experiments, alters outcomes, re-ranks experiments, or recomputes the Phase-17 engineering value. No ML/stats/NLP/AI/network.

**ADDITIVE, IDEMPOTENT migration — `DB_VERSION` 25 → 26** (new standalone metadata-only `engineering_campaign_registry` table; the program's first migration since Phase 11; NO existing table/column/query altered), `RULE_ENGINE_VERSION` 46.0. NEW pure `strategy/evidence_saturation.py` (`EvidenceSaturation` NOT_STARTED/EARLY/BUILDING/STRONG/SATURATED/OVERTESTED + `assess_saturation`; every count in `signals`, every threshold a named visible constant, every status carries `reasons`, `information_gain_remaining` high/moderate/low/none; a fixed dead OVERTESTED-conflicting branch — was unreachable — and hardened non-mapping/garbage handling). NEW pure `strategy/engineering_cost_model.py` (`estimate_experiment_cost` A/B/A laps=warmup+baseline+test(+coupled)+revert, time/fuel/tyres, value_per_lap/value_per_minute/info_gain_per_tyre_set — **value reused VERBATIM from Phase 17**; `EngineeringBudget`/`plan_budget` deterministic GREEDY fit in Phase-17 rank order, NOT an optimiser). NEW pure `strategy/campaign_persistence.py` (`CampaignRegistryEntry` metadata-only + `campaign_age_days` [dates are DATA] + `build_engineering_efficiency` composing age+saturation+cost+budget, deterministic fingerprint). NEW pure `strategy/engineering_efficiency_render.py`. The ONLY new write: `SessionDB.record_engineering_campaigns` (idempotent — INSERT OR IGNORE preserves first_seen/creation_session, UPDATE refreshes last_seen/last_updated/completion_state/linked_experiments; user notes/archive NEVER clobbered) + `get_campaign_registry`/`set_campaign_note`; `SessionDB.build_engineering_efficiency` (READ-ONLY by default; reuses `build_engineering_campaign_programme` ONCE + one registry read; the registry capture is OPT-IN only when `register_session_id` is passed, best-effort, touches ONLY the registry). UI: `ui/engineering_efficiency_vm.py` + `ui/engineering_efficiency_panel.py` (`EngineeringEfficiencyPanel`, NO Apply/Approve/Freeze/Complete/Execute/edit controls — asserted) in Development History beneath the Phase-18 panel; build OFF the Qt thread via the reused `MechanismAnnotationWorker` (dashboard supplies session id + today's date → the off-thread build performs the additive registry capture). **NEW suites `tests/test_phase19_{saturation,cost,persistence,golden,query_shape,safety,ui_construction}.py` (66)** — all green; Phase 8–18 version-affected suites (260) re-green after the DB_VERSION 25→26 bump (assertions updated 25→26); no-AI architecture + config-safety guards green. Golden `config_id` + frozen allowlist + Apply-gate + engine-wiring-status UNCHANGED; protected runtime files git-verified byte-identical. Doc: `docs/ENGINEERING_BRAIN_PHASE19_CAMPAIGN_PERSISTENCE.md`. **Recommended next: Phase 20 — confidence-weighted evidence & development ROI (not started).**

---

## Prior Objective (2026-07-19) — Engineering Brain PROGRAM 2, Phase 18: Engineering Campaigns & Multi-Session Development Planning — COMPLETE
