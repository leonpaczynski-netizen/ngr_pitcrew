...
### Phase 5 — What the telemetry literature adds (written 14 Sep 2026, revision 10 — 5.B0 accepted and applied; 5.19 guided practice added, critic pending on 5.19)
...
| # | Work | Answers | Done when |
|---|---|---|---|
| 5.B0 | ✅ **ACCEPTED 14 Sep 2026 — his yes to the sentence and both parts; applied to the contract files (RECONCILIATION AV) and pinned by `pitcrew/tests/test_best_corner_rule.py`.** The amendment, in two parts, with one sentence explaining the edit to his words. Filed in `brain/RECONCILIATION.md`. **The sentence he is told:** *"You asked for the recipe off the laps that were fastest; the fastest laps are partly luck, so instead I work out which input paid, and where I cannot tell I say so and give you a test to try. With the laps we have today, most corners will start as a test, not a recipe, and many tests will need more laps than a session gives."* His yes to that change is recorded. **Part 1 — his words, which are what he hears:** *"The engineer builds a best corner from your own laps. For every corner it looks at entry, middle and exit together — where you braked, how hard, how much you turned, your line and where you got back on the throttle — and works out, where your laps can show it, which of those bought you apex speed and exit speed and which cost you. Where they pull against each other it says so in your terms: 'the later brake got you apex speed but lost the exit — try braking a touch earlier here'. Every marker says how many of your laps it came from and how sure it is, and a corner without enough laps says 'can't tell yet' and gives you a test instead. A new marker is tried, back to the old one, and again, and becomes the reference when the time down the next straight and your feel agree — and if they disagree, that is what gets written down."* **Part 2 — the guards, written in full into the `CLAUDE.md` header, charter §2, `references/refusals.md` (:17–31, :112–116) and `project_corner_noise_floor_2026_08_22`:** a per-lap per-corner instruction stays refused; a reference is `[DERIVED]`, built one input at a time with arriving speed, fuel and the session held on **both** the input and the result, never from the laps with the best result; **an input is credited alone only when enough matched laps separate it from what moves with it — otherwise it is reported as that input and what moves with it, or "can't tell yet"**; laps on an unknown setup are not used; laps are compared only within one car × circuit × game version × compound × multiplier; **laps on different setups are pooled only on the stated assumption that an input pays the same on each, never for brake-phase inputs across a change to brake balance, `lsd_b` or any key the ledger tags brake-phase, never for throttle-phase inputs across `lsd_a` or any key tagged traction, and that assumption is named in every sentence built on it**; the input examined is chosen before looking, and anything beyond one input per corner, or across corners, is corrected for how many were tried; a trial is sized from that corner's floor before he drives, refused when the laps exceed what he will run, and judged on segment time against its floor **and** his report; a corner is named only on a model that is `track-map` in the form the export reads (5.2b); in practice George may guide a best lap from markers Ludo issued; he never turns one lap into a marker or a verdict, and never calls markers in a race (his answer, 14 Sep: *"yes guided best lap in practice"*). **Nothing in 5B that names a corner or issues a marker starts until he says yes to the sentence and both parts** | the driver's dispute, his wording, critic passes 6–9 | his yes or no recorded; if yes, the four files carry both parts and the tests pinning the refusal card change in the same commit |
...
| 5.4a | **Which setup each lap was driven on, readable by a tool — without the app holding a setup.** A normalised table in `brain/ledger/<car>-<circuit>.md` keyed on **session plus lap range** → car-state revision id (a link, never a value), with **`unknown`** allowed — brake balance moves on the MFD mid-session and session 147 carried a change of his own beside the issued one — and laps on an `unknown` revision are ineligible for 5.5. Each ledger key carries a **phase tag** (brake / traction / platform / aero) used by 5.5's pooling rule. The free-text `session_ids` are resolved once by hand from the car-state files, with the driver asked where they cannot be. **The database gains nothing** (§1a) — the tool joins brain to DB at read time | critic passes 6–7: no lap→revision join exists; setups change inside sessions | a tool prints eligible laps per corner per revision for Daytona, Sardegna and Spa, with the unknown laps counted separately |
| 5.4 | **The input map.** On the fitted map, per corner: brake-on, brake release, turn-in, apex, throttle pickup and full-throttle points as marks, **the band around each is its uncertainty, printed with n — not a consistency score, never spoken** (row 2.11); speed colour along the lap; **line as the lateral offset from his median path, from `pos_x`/`pos_z`**. Two uses: the best-corner reference from 5.5 drawn against a session's laps, and run A vs run B for a change or a marker test. Names from the hub, disagreements shown not resolved, only where 5.B0 and 5.2b allow a name. A difference inside 5.3's floor is drawn grey and says "can't tell yet". **The drawing and its `pos_x`/`pos_z` stay in the app — nothing of it reaches the export or MCP** (rule 2, §8) | the driver: *"overlay throttle and braking onto the map for analysis"* | a Daytona session drawn with n and bands; one corner reads "can't tell yet" |
| 5.5 | **The best-corner builder — his request of 14 Sep, after 5.B0's yes, 5.3's floors and 5.4a.** **Eligible laps:** frames, not excluded, `off_track_s` < 1.0, no penalty, no teleport, no struck kerb (`corners.py:845`), a known revision (5.4a), one car × circuit × game version × compound × multiplier. **On file today, before the revision split:** Daytona Huracán RS 113 over 23 sessions; Sardegna RSR RH 41 over 7 and RM 30 over 3; Spa Huracán RS 48 over 11 — about 3–13 per revision at Daytona. **The judge** is time from a fixed point before the earliest brake-on to a fixed point down the next straight, with entry, middle and exit cut at fixed distances. **Inputs** (what he does): brake-on point, peak brake %, brake release, turn-in point, lock at turn-in and apex from `road_wheel_rad`, line as lateral offset from his median path, throttle pickup point, throttle build. **Outcomes** (what the car gave back): apex speed, exit speed, rotation with opposite-lock frames, rear wheelspin. **Method.** (1) **Choose before looking:** for each corner, the one input to examine is picked from the phase where the fixed-distance split says the time goes and from his report, **before any slope is computed**; the false-discovery control runs **across every slope examined at the circuit** (all corners × outcomes), not per corner, and the count examined and the expected false clears are printed. (2) **Hold what the corner does not control — on both sides:** the outcome (segment time, apex or exit speed) **and the input** are each residualised on arriving speed, fuel and a per-session offset (which absorbs day, wind and track; lap in stint is dropped as near-collinear with fuel inside a run), then a **Theil-Sen slope of residual on residual**, with its interval and n. A single revision (3–13 laps over 1–3 sessions) barely identifies a slope after the session offsets, so **in practice the slope quoted will be the pooled one of step 4**, and that is said. (3) **Matched pairs where they exist:** two laps in one session within the 5.3 floor of arriving speed and of every other input, differing in this one by more than its floor; **at least four pairs**, else the *pair check* reads **"can't tell yet"** (the slope is then worded as below, never as a single-input credit). A slope is credited **to the input alone** only when it clears **and at least four pairs agree**; with fewer than four pairs a clearing slope reads **"this input and what moves with it"** (named: every input whose residual correlates with it at |r| ≥ 0.5, with n), and pairs that disagree report the inputs as a group. At about 5 laps a session expect well under one usable pair per session, so single-input credit will be rare until laps accumulate. **At today's counts most corners will read "can't tell yet"**, and that is the expected state, not a failure. (4) **Pooling across setups:** a per-revision offset with one shared slope, **`[ASSUMED]`** and named in the sentence; never brake-phase inputs across a brake-tagged change, never throttle-phase inputs across a traction-tagged one (5.4a tags); the slope-difference test runs when two revisions each hold **at least 20 eligible laps at that corner**, and until then the assumption is unrefuted, not confirmed, and said so — and even at 20 the test is low-powered, so "not refuted" is weak evidence and is worded that way; at Daytona's rate of setup change it may never run. **The trade-off sentence** comes from the apex and exit slopes of the same input and is **something to try, not a finding**. **What the builder mostly delivers is the test**: the confirmation run — tried, back, again — **sized from the slope shrunk halfway toward zero** — which roughly quadruples the laps, so many trials will be refused, and he is told so —  refused when the laps exceed what he will run, judged on segment time against its floor **and his report**, a disagreement recorded as the finding (rule 1). **[MEASURED]** the inputs and times; **[DERIVED]** every slope, marker and sentence. Delivered in Ludo's brief and on the 5.4 map | the driver, 14 Sep: *"when you broke here applying x% brake and turned this much and got on throttle here that was fastest apex speed and best exit"* | at one corner: the pre-chosen input, its residualised slope with n and interval, the pair check or "can't tell yet", a sentence labelled to try, and a confirmation run with a verdict row; at a corner without enough laps: "can't tell yet" and the test it would take |
| 5.6 | **Throttle and steering inside 5.5, not beside it.** Per-lap `throttle_on_pct` 2σ is 11–51 pp, so a throttle marker will often be the widest band on the map; 5.3 says where it pools to something sayable and 5.5 draws the rest grey. Steering is judged as lock at turn-in and apex, with opposite-lock frames, never as a smoothness score | IODA throttle application | 5.3's throttle and steering columns name the corners, or none |
| 5.19 | **Guided best lap in practice — his answer of 14 Sep, *"yes guided best lap in practice"*. Critic pass required before it is built.** George is race-gated by design (every proactive call path bails without a running race); the **rehearsal** seam (`sessions.rehearsal`) arms the engine without a race and is the path to reuse, not a fourth session kind. In a guided practice George speaks **only markers Ludo issued for that session's run plan** (5.5's reference or its try marker), one short line before the corner with the marker and what the run is testing, and **says the try / back / again switch on the lap it happens**. The run's result is said **when the run's laps are in**, never per lap. **Never** a marker or a verdict built from one lap; **never** in a race; silent after an off (`composure`). **Whether he may say where the driver braked against the marker on that lap** — a measured position, not an instruction — **is decided by 5.3's evidence** that a single lap's brake-on point is placed within the marker's tolerance on that circuit's model; until then he does not. Corners are called by name only on a circuit that has passed 5.2b. Every guided line is filed with its marker id and run so the verdict can be audited (§5.5) | the driver, 14 Sep | a harness replay of a practice with an issued marker set speaks each marker line before its corner, the switch on the right laps, nothing on an off lap, no unissued marker, and nothing in a race session |
...
| 5.18 | **The teammate comparison with the data we have.** No telemetry from his car. The hub's `RoundResult` gives quali and best race lap per round → pace gap per round and circuit, with n, in the brief. **Setups (driver, 14 Sep): similar but not the same, each developed for its own driver's style** — so a teammate's sheet is a **source of hypotheses, never a value to copy**: where it differs on an axis in percent of range — **computed against the teammate car's own range record, and comparable only when it is the same car** — that difference is a `what to try` proposal that passes the refusal card's gates like any other (a floor, the laps, one change), tested on his car. It is kept under `brain/teammates/`, **never in `brain/car-state/` and never in the database** (§1a). **A shared replay is replay-derived data, out of scope under `CLAUDE.md` §8** — not planned unless he amends §8 | EngineStories teammate comparison; the driver's answer | hub gap per round in the pre-race brief; a teammate sheet, when shared, yields proposals with falsifiers and no copied values |
...
with its WAL; then `--apply` circuit by circuit outside its race week; 5.4a, 5.4,
...
**Revision 6 (driver, 14 Sep).** He answered 5.B0 with the rule as he wants it written:
build a best corner from entry, middle and exit together — brake point and %, steering,
line, throttle point — judged on apex speed and exit, with the trade-off said in his
terms. 5.B0 now carries his wording for his final yes; 5.5 is rebuilt as the best-corner
builder; 5.4 and 5.6 serve it; 5.18 takes his answer that teammate setups are similar but
developed for each driver.

**Revision 7 answers critic pass 6 (NOT AGREED):** B1 the reference is no longer read
off the laps with the best result — one input at a time, arriving speed held, matched
pairs as the check, apex speed and rotation moved to outcomes, the trade-off labelled
to try; B2 real eligible counts quoted and the revision split named, with 5.4a as the
tool-readable session→revision map in `brain/` and a stated `[ASSUMED]` shared slope
across revisions, tested; M1 5.B0 split into his words and the guards the card
carries; M2 turning from `road_wheel_rad`, traffic held out in races only. Minors:
phases cut at fixed distances; 5.18 against the teammate car's range record, under
`brain/teammates/`, through the card's gates.

**Revision 8 answers critic pass 7 (NOT AGREED):** M1 pair tolerance and a four-pair
minimum stated, "no pairs" is "can't tell yet", Theil-Sen on residuals; M2 one input per
corner chosen before looking, a false-discovery control beyond that, trials sized from a
halved slope; M3 fuel, lap in stint (later dropped, rev 9) and a per-session offset restored; M4 5.4a keyed on
session plus lap range with `unknown`, phase tags, no brake inputs pooled across a
brake-tagged change nor throttle across a traction-tagged one, the slope test at 20 laps
a revision. Minors: the sentence telling him why his words were edited, and Part 2 made
self-contained.

**Revision 9 answers critic pass 8 (NOT AGREED, two one-line defects):** M1 input and
outcome both residualised before Theil-Sen; M2 below four agreeing pairs a slope reads
"this input and what moves with it", never a single-input credit, and Part 2 carries
that guard. Minors: stint lap dropped for fuel, the pooled slope named as the usual
case, the slope test called low-powered, false-discovery control across the circuit,
the driver told most corners start as tests and many trials will be refused, Part 2
gains the unknown-revision and judged-with-his-report guards.

**Critic pass 9: AGREED.** Its four minors are folded in (pair check vs slope wording,
|r| ≥ 0.5 for "what moves with it", the 5.B0 citation, the rev-8 record annotated).

**Revision 10 (14 Sep).** He said yes to 5.B0 and yes to a guided best lap in practice.
The amendment is applied to `CLAUDE.md`, charter §2, the refusal card, `gt7-brain`,
