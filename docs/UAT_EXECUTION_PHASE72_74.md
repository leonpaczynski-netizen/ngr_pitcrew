...
| DEF-UAT-073-001 | Medium (usability) | desktop_visual_layout, live_dashboard_readability | **LARGELY FIXED** — the legacy Home dashboard was removed (`e24c7c6`); the biggest offender (Setup Builder proportions) was reworked: Shift-RPM compacted, recommendation given a readable splitter share, locked tuning sections hidden (progressive disclosure), and a compact READ-ONLY "Transcribe to GT7" view (`<new>`) so the whole setup is visible at a glance without scrolling the tall editor; the Live tab is now scroll-wrapped + mode-differentiated (DEF-017). Remaining proportion polish is per-tab and can be handled as observed. **User retest.** |
...
| DEF-UAT-073-007 | Medium (usability) | desktop_visual_layout (Setup Builder) | **LARGELY FIXED** — Shift-RPM box compacted to a single row (was an oversized multi-row block); the recommendation pane now takes the larger share of a vertical splitter when present and collapses when absent (readable, no longer a squished strip); and **progressive disclosure** now HIDES tuning sections the Event locks out entirely (both forms) so you only scroll what you can edit. Remaining: the side-by-side Race+Qualifying "Both" editor is still space-heavy for at-a-glance transcription — a compact single-setup transcribe view is a possible follow-up. **User retest.** |
| DEF-UAT-073-008 | High (functional) | Setup Builder recommendation→car-settings mapping | **FIXED** in `<new>` — user repro: baseline build, Race column, most fields didn't load and ride height loaded wrong (only fields already at their default looked right). Root cause confirmed: after the structured-recommendation-view refactor, `_display_setup_result` rendered the rec view and *highlighted* fields but NEVER wrote values into the Car Setup form for a baseline — so every field kept its default (ride height = 80 mm default, not the authored baseline). Secondary: the rec key `brake_bias` was dropped because `fill_setup_fields` reads `brake_bias_front`. Fixes: (1) baseline builds (`entry_type == "baseline_setup"`) now fill the target form from the category-gated `setup_fields`; the Analyse path stays Apply-gated; (2) `apply_ai_fields` translates `brake_bias`→`brake_bias_front`. 4 tests. **User retest.** |
| DEF-UAT-073-009 | High (functional / dead controls) | Setup Builder Apply-in-Game / value entry / Start Validation | **FIXED** in `<new>` — beyond the visible feedback line (`44a6902`): (1) every action now gates on `has_recommendation` — Validate/Feedback/Accept&Lock used to sit in their default-enabled state so they looked clickable with nothing loaded and did nothing; (2) Start Validation now makes an in-view change (header Status → "Validating", focuses the Test Plan tab) instead of only refreshing off-view state. The FROZEN Apply-gate predicate is untouched (control enable/display only). 3 tests. **User retest.** |
...
| DEF-UAT-073-017 | Medium (layout + redundancy) | live_dashboard_readability (Live Race Engineer tab) | **FIXED** in `<new>` — (1) the tab content is now wrapped in a QScrollArea (widget-resizable, no horizontal scroll) so the mode-specific panel and bottom content are never clipped on limited-height / VR windows, and the mode stack gets a stretch factor so it isn't squished; (2) a prominent colour-coded mode banner (RACE / PRACTICE / QUALIFYING + purpose) names the active mode, so Race and Qualifying are unmistakably distinct. 4 tests. **User retest.** |
