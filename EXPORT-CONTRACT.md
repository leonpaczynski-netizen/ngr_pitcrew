...
| `verdict` | Meaning |
|---|---|
| `acted` | A stop followed a box call inside its own window, or none followed a stay-out fold — one instrument, `laps.is_pit_lap`, answering two questions. `verdictDetail` names the lap |
...
| `borne-out` | **A report, not an instruction**, and the laps on file show what it said: a place called matches `laps.position` at the line that closed its lap; "two to go" / "last lap" matches the lap the flag fell on; a fuel shortfall to the flag ended with the tank dry. Says nothing about the driver |
| `not-borne-out` | A report the laps on file do not bear out. **Not a claim the call was false**: a place held for eight seconds and lost before the line reads the same as one that was never his, and a shortfall that did not arrive may be fuel he saved because he was asked to. `verdictDetail` says what the laps show and, where they cannot separate two readings, says so. Added in 1.9, 13 Sep 2026 |
| `cannot-tell` | Nothing on file can answer this kind of call, or the race ended before the window was driven. `verdictDetail` gives **the reason for that kind** — a brake-balance click is in no packet; a green, a tyre-temperature report or an incident strike is the same reading the laps are built from; a claim about the flag in a race ended without one. **A short-shift call is one of these**: `laps.short_shift_rpm` is the app's own switch and not the driver's hand, so judging on it asked whether the app did what the app did — and `laps.upshift_rpm`, which does read his hand, has only been calibrated on him STOPPING a saving, not starting one |
...
**"Instruction" here means a box call, and only a box call.** A short-shift
call is an instruction the driver hears and is not one for these two fields:
nothing on file reads whether he took it, so its `verdict` is `cannot-tell`
and its `disposition` stays `informational`, claiming nothing either way.

**Where `verdict` and `disposition` are both present for a box call they
