"""Phase 19 — query-shape: one efficiency build, bounded query count, no N+1; renderer DB-free."""
import pytest
...
def test_empty_efficiency_cheap(tmp_path):
    ...
    assert db._conn.selects <= 20
    db.close()


def test_renderer_touches_no_db(tmp_path):
    from strategy.engineering_efficiency_render import render_efficiency_text
    db = SessionDB(str(tmp_path / "r.db")); _seed(db, 2)
    result = db.build_engineering_efficiency(applied_setup=applied(), now_date="2026-07-10",
                                             **_kw())
    db._conn = _CountingConn(db._conn)
    render_efficiency_text(result["efficiency"])
