"""Phase 21 — query-shape: one build, bounded query count, no N+1; renderer DB-free."""
import pytest
...
def _seed(db, n):
    ...
    for i in range(n):
        ...
        rec = build_development_record(
            ...
            residuals=[{"issue_key": f"k{i}", "family": fam,
...
def test_empty_season_cheap(tmp_path):
    ...
    assert db._conn.selects <= 26
    db.close()


def test_renderer_touches_no_db(tmp_path):
    from strategy.season_engineering_report_render import render_report_text
    db = SessionDB(str(tmp_path / "r.db")); _seed(db, 3)
    result = db.build_season_engineering_report(applied_setup=applied(), now_date="2026-07-10",
                                                **_kw())
    db._conn = _CountingConn(db._conn)
    render_report_text(result["season_report"])
