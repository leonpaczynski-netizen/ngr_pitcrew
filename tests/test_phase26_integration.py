"""Phase 26 — integration / query-shape tests.

Phase 22 built at most once via the shared knowledge chain; Phase 23 + Phase 24 + Phase 25 SessionDB
entries never called; in-memory reuse; constant query count vs event count; renderer zero DB access;
no writes; DB hash / table counts / user_version unchanged.
"""
...
def test_query_count_constant_in_event_count(tmp_path):
    ...
    db1.build_programme_revalidation_report(applied_setup=applied(), now_date="2026-07-10", **_kw())
    c1 = db1._conn.selects
    db6 = SessionDB(str(tmp_path / "b.db")); _seed(db6, 6)
    db6._conn = _CountingConn(db6._conn)
    db6.build_programme_revalidation_report(applied_setup=applied(), now_date="2026-07-10", **_kw())
    c6 = db6._conn.selects
    assert c1 == c6, f"N+1 detected (query count grew with events): {c1} -> {c6}"
    db1.close(); db6.close()


def test_phase22_once_phase23_24_25_db_never_called(monkeypatch, tmp_path):
    ...
    db.build_programme_revalidation_report(applied_setup=applied(), now_date="2026-07-10", **_kw())
    assert calls["pk"] == 1, f"Phase-22 built {calls['pk']} times (must be 1)"
    assert calls["transfer_db"] == 0, "Phase-23 DB entry must not be called"
    assert calls["playbook_db"] == 0, "Phase-24 DB entry must not be called"
    assert calls["timeline_db"] == 0, "Phase-25 DB entry must not be called"
    db.close()
...
def test_renderer_touches_no_db(tmp_path):
    ...
    result = db.build_programme_revalidation_report(applied_setup=applied(), now_date="2026-07-10",
                                                    **_kw())
    db._conn = _CountingConn(db._conn)
    render_revalidation_text(result["revalidation"] or {})
...
def test_empty_cheap(tmp_path):
    ...
    db._conn = _CountingConn(db._conn)
    r = db.build_programme_revalidation_report(car=PORSCHE, track="Fuji", discipline="Race")
    assert r["ok"] and r["domain_count"] == 0
...
def test_result_shape(tmp_path):
    ...
    r = db.build_programme_revalidation_report(applied_setup=applied(), now_date="2026-07-10", **_kw())
    assert set(r) >= {"ok", "revalidation", "domain_count", "content_fingerprint"}
    assert r["ok"] and r["domain_count"] >= 1
    assert r["content_fingerprint"] == r["revalidation"]["content_fingerprint"]
    db.close()
