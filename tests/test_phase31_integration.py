"""Phase 31 — integration / query-shape tests.

Phase 22 built at most once via the shared knowledge chain; Phase 23/24/25/26/27/28/29/30 SessionDB
...
def test_query_count_constant_in_event_count(tmp_path):
    ...
    db1.build_programme_assurance_report(applied_setup=applied(), now_date="2026-07-10", **_kw())
    c1 = db1._conn.selects
    db6 = SessionDB(str(tmp_path / "b.db")); _seed(db6, 6)
    db6._conn = _CountingConn(db6._conn)
    db6.build_programme_assurance_report(applied_setup=applied(), now_date="2026-07-10", **_kw())
...
def test_phase22_once_lower_db_entries_never_called(monkeypatch, tmp_path):
    ...
    db.build_programme_assurance_report(applied_setup=applied(), now_date="2026-07-10", **_kw())
    assert calls["pk"] == 1
    for name in lower:
...
def test_renderer_touches_no_db(tmp_path):
    ...
    result = db.build_programme_assurance_report(applied_setup=applied(), now_date="2026-07-10",
                                                 **_kw())
    db._conn = _CountingConn(db._conn)
    render_assurance_text(result.get("assurance") or {})
...
def test_empty_cheap(tmp_path):
    ...
    db._conn = _CountingConn(db._conn)
    r = db.build_programme_assurance_report(car=PORSCHE, track="Fuji", discipline="Race")
    assert r["ok"] and r["finding_count"] == 0 and r["grade"] == "insufficient_evidence"
...
def test_result_shape(tmp_path):
    ...
    r = db.build_programme_assurance_report(applied_setup=applied(), now_date="2026-07-10", **_kw())
    assert set(r) >= {"ok", "assurance", "grade", "finding_count", "content_fingerprint"}
    assert r["ok"]
    assert r["content_fingerprint"] == r["assurance"]["content_fingerprint"]
    assert r["grade"] == r["assurance"]["assurance_grade"]
    db.close()


def test_no_structural_non_determinism_from_real_pipeline(tmp_path):
    """The real sub-reports all carry fingerprints, so the audit must NOT raise a
    non-deterministic-output finding."""
    db = SessionDB(str(tmp_path / "n.db")); _seed(db, 3)
    r = db.build_programme_assurance_report(applied_setup=applied(), now_date="2026-07-10", **_kw())
    types = {f["finding_type"] for f in (r.get("assurance") or {}).get("findings", [])}
    assert "non_deterministic_output" not in types
    assert "data_mutation_detected" not in types
    db.close()
