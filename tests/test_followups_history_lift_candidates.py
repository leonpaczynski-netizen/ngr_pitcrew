...
def test_baseline_response_no_history_no_lift():
    ...
    # Camber is now a car/objective-specific chassis seed (calibrated to reference
    # tunes: front > rear, ~2.4 for a high-downforce car on a Race objective). The
    # point of THIS test is that with no history there is no proven-setup lift.
    assert res["setup_fields"]["camber_front"] == 2.4
