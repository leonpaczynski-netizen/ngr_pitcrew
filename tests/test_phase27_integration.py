"""Phase 27 — integration / query-shape tests.

Phase 22 built at most once via the shared knowledge chain; Phase 23/24/25/26 SessionDB entries
...
def test_query_count_constant_in_event_count(tmp_path):
    ...
    db1.build_programme_evidence_coverage_report(applied_setup=applied(), now_date="2026-07-10",
                                                 **_kw())
    c1 = db1._conn.selects
    db6 = SessionDB(str(tmp_path / "b.db")); _seed(db6, 6)
    db6._conn = _CountingConn(db6._conn)
    db6.build_programme_evidence_coverage_report(applied_setup=applied(), now_date="2026-07-10",
                                                 **_kw())
...
def test_phase22_once_lower_db_entries_never_called(monkeypatch, tmp_path):
    ...
    db.build_programme_evidence_coverage_report(applied_setup=applied(), now_date="2026-07-10",
                                                **_kw())
    ...
    assert calls["reval_db"] == 0, "Phase-26 DB entry must not be called"
    db.close()
...
def test_renderer_touches_no_db(tmp_path):
    ...
    result = db.build_programme_evidence_coverage_report(applied_setup=applied(),
                                                         now_date="2026-07-10", **_kw())
    db._conn = _CountingConn(db._conn)
    render_coverage_text(result["coverage"] or {})
...
def test_empty_cheap(tmp_path):
    ...
    db._conn = _CountingConn(db._conn)
    r = db.build_programme_evidence_coverage_report(car=PORSCHE, track="Fuji", discipline="Race")
...
def test_result_shape(tmp_path):
    ...
    r = db.build_programme_evidence_coverage_report(applied_setup=applied(), now_date="2026-07-10",
                                                    **_kw())
    assert set(r) >= {"ok", "coverage", "domain_count", "blind_spot_count", "content_fingerprint"}
    assert r["ok"] and r["domain_count"] >= 1
    assert r["content_fingerprint"] == r["coverage"]["content_fingerprint"]
    db.close()


def test_chain_records_do_not_leak_setup_values(tmp_path):
    import json
    import re
    db = SessionDB(str(tmp_path / "l.db")); _seed(db, 3)
    r = db.build_programme_evidence_coverage_report(applied_setup=applied(), now_date="2026-07-10",
                                                    **_kw())
    blob = json.dumps(r["coverage"])
    assert not re.search(r'"(arb_front|lsd_accel)"\s*:\s*-?\d', blob)
