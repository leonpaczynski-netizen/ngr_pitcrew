"""Phase 20 — query-shape: one build, bounded query count, no N+1; renderer DB-free."""
import pytest
...
def _seed(db, n):
    for i in range(n):
        rec = build_development_record(
            {"id": str(i), "experiment_id": 100 + i, "status": "confirmed_improvement",
...
def test_quality_query_count_constant(tmp_path):
    db1 = SessionDB(str(tmp_path / "a.db")); _seed(db1, 1)
    db1._conn = _CountingConn(db1._conn)
    db1.build_engineering_knowledge_quality(applied_setup=applied(), now_date="2026-07-10", **_kw())
    c1 = db1._conn.selects
    db5 = SessionDB(str(tmp_path / "b.db")); _seed(db5, 5)
    db5._conn = _CountingConn(db5._conn)
    db5.build_engineering_knowledge_quality(applied_setup=applied(), now_date="2026-07-10", **_kw())
    c5 = db5._conn.selects
    assert c1 == c5, f"N+1 detected (query count grew with campaigns): {c1} -> {c5}"
    db1.close(); db5.close()


def test_empty_quality_cheap(tmp_path):
    ...
    assert db._conn.selects <= 24
    db.close()


def test_renderer_touches_no_db(tmp_path):
    from strategy.engineering_knowledge_quality_render import render_quality_text
    db = SessionDB(str(tmp_path / "r.db")); _seed(db, 2)
    result = db.build_engineering_knowledge_quality(applied_setup=applied(),
                                                    now_date="2026-07-10", **_kw())
    db._conn = _CountingConn(db._conn)
    render_quality_text(result["knowledge_quality"])
