...
def test_schema_version_is_v10(db):
    ...
    # Reconciled for Engineering-Brain Phase 3: 21 → 22 (_migrate_v22 added the five standalone
    ...
    # Reconciled for Program 3 legacy classification: 37 → 38 (_migrate_v38 classifies legacy
    # sessions + adds the read-only quarantine_records view).
    # Reconciled for UAT 2026-08-07 defect B4: 40 → 41 (_migrate_v41 widens
    # driver_feedback to the full 14-field Practice Review capture set — nine fields
    # had no column and were dropped on every write).
    # The test name is kept stable to not disrupt git blame.
    from strategy._setup_constants import DB_VERSION
    version = db._conn.execute("PRAGMA user_version").fetchone()[0]
    assert version == DB_VERSION == 41
