"""Phase 29 — integration / query-shape tests.

Phase 22 built at most once via the shared knowledge chain; Phase 23/24/25 SessionDB entries never
called; in-memory reuse; constant query count vs event count (no N+1); renderer zero DB access; no
writes; DB hash / table counts / user_version unchanged.
"""
...
def test_query_count_constant_in_event_count(tmp_path):
    ...
    db1.build_programme_contradiction_report(applied_setup=applied(), now_date="2026-07-10", **_kw())
    c1 = db1._conn.selects
    db6 = SessionDB(str(tmp_path / "b.db")); _seed(db6, 3, 6)
    db6._conn = _CountingConn(db6._conn)
    db6.build_programme_contradiction_report(applied_setup=applied(), now_date="2026-07-10", **_kw())
...
def test_phase22_once_lower_db_entries_never_called(monkeypatch, tmp_path):
    db = SessionDB(str(tmp_path / "c.db")); _seed(db)
    calls = {"pk": 0, "transfer_db": 0, "playbook_db": 0, "timeline_db": 0}
    ...
    db.build_programme_contradiction_report(applied_setup=applied(), now_date="2026-07-10", **_kw())
    assert calls["pk"] == 1
    for k in ("transfer_db", "playbook_db", "timeline_db"):
...
def test_renderer_touches_no_db(tmp_path):
    ...
    db = SessionDB(str(tmp_path / "r.db")); _seed(db)
    result = db.build_programme_contradiction_report(applied_setup=applied(), now_date="2026-07-10",
                                                     **_kw())
    db._conn = _CountingConn(db._conn)
    render_contradiction_text(result.get("contradiction") or {})
...
def test_no_writes_db_hash_and_counts_unchanged(tmp_path):
    ...
    assert db._conn.execute("PRAGMA user_version").fetchone()[0] == uv_before == DB_VERSION
    db.close()
...
def test_empty_cheap(tmp_path):
    ...
    db._conn = _CountingConn(db._conn)
    r = db.build_programme_contradiction_report(car=PORSCHE, track="Fuji", discipline="Race")
    assert r["ok"] and r["contradiction_count"] == 0
    assert db._conn.selects <= 40
    db.close()


def test_result_shape_and_detects_contradiction(tmp_path):
    db = SessionDB(str(tmp_path / "s.db")); _seed(db, 3, 2)
    r = db.build_programme_contradiction_report(applied_setup=applied(), now_date="2026-07-10",
                                                **_kw())
    assert set(r) >= {"ok", "contradiction", "contradiction_count", "open_count",
                      "content_fingerprint"}
    assert r["ok"] and r["contradiction_count"] >= 1
    assert r["content_fingerprint"] == r["contradiction"]["content_fingerprint"]
    db.close()
