"""Phase 25 — integration / query-shape tests.

Phase 22 built at most once; Phase 23 + Phase 24 SessionDB entries never called; in-memory reuse;
constant query count vs event/campaign count; renderer zero DB access; no writes; DB hash / table
counts / user_version unchanged.
"""
...
def test_query_count_constant_in_campaign_count(tmp_path):
    ...
    db1.build_programme_knowledge_timeline(applied_setup=applied(), now_date="2026-07-10", **_kw())
    c1 = db1._conn.selects
    db6 = SessionDB(str(tmp_path / "b.db")); _seed(db6, 6)
    db6._conn = _CountingConn(db6._conn)
    db6.build_programme_knowledge_timeline(applied_setup=applied(), now_date="2026-07-10", **_kw())
    c6 = db6._conn.selects
    assert c1 == c6, f"N+1 detected (query count grew with campaigns): {c1} -> {c6}"
    db1.close(); db6.close()


def test_phase22_once_phase23_24_db_never_called(monkeypatch, tmp_path):
    db = SessionDB(str(tmp_path / "c.db")); _seed(db, 3)
    calls = {"pk": 0, "transfer_db": 0, "playbook_db": 0}
    ...
    db.build_programme_knowledge_timeline(applied_setup=applied(), now_date="2026-07-10", **_kw())
...
def test_renderer_touches_no_db(tmp_path):
    ...
    result = db.build_programme_knowledge_timeline(applied_setup=applied(), now_date="2026-07-10",
                                                   **_kw())
    db._conn = _CountingConn(db._conn)
    render_timeline_text(result["timeline"] or {})
...
def test_empty_cheap(tmp_path):
    ...
    assert r["ok"] and r["point_count"] == 0
