"""Phase 18 — query-shape tests: one portfolio build, bounded query count, no N+1."""
import pytest
...
def test_campaign_query_count_constant(tmp_path):
    db1 = SessionDB(str(tmp_path / "a.db")); _seed(db1, 1)
    db1._conn = _CountingConn(db1._conn)
    db1.build_engineering_campaign_programme(applied_setup=applied(), **_kw())
    c1 = db1._conn.selects
    db5 = SessionDB(str(tmp_path / "b.db")); _seed(db5, 5)
    db5._conn = _CountingConn(db5._conn)
    db5.build_engineering_campaign_programme(applied_setup=applied(), **_kw())
    c5 = db5._conn.selects
    assert c1 == c5, f"N+1 detected (query count grew with diagnoses/campaigns): {c1} -> {c5}"
    db1.close(); db5.close()


def test_empty_campaign_cheap(tmp_path):
    ...
    assert db._conn.selects <= 16
    db.close()


def test_renderer_touches_no_db(tmp_path):
    from strategy.engineering_campaign_render import render_programme_text
    db = SessionDB(str(tmp_path / "r.db")); _seed(db, 2)
    result = db.build_engineering_campaign_programme(applied_setup=applied(), **_kw())
    db._conn = _CountingConn(db._conn)
    render_programme_text(result["programme"])
