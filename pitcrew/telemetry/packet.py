...
PACKET_SIZE_NEW = 368   # 'C' format; same base layout, 72 extra bytes

# GT7 emits one of four formats, selected by the heartbeat character sent to the
# console.  Every format shares the 296-byte base; the tail differs.
#
#   'A'  296 B  base set
#   'B'  316 B  + wheel rotation, sway, heave, surge
#   '~'  344 B  + per-wheel surface type, steering angles, wheelbase,
#               filtered pedals, torque vectors, energy recovery
#   'C'  368 B  + current lap time in ms, car category
#
# We request 'C': it is a superset, and it is the only format carrying current
# lap time in milliseconds, which live strategy needs.  The export has to
# declare which format was captured (`meta.packet`) so that a null channel
# reads as "not available in this format" rather than "not measured".
PACKET_FORMATS: dict[int, str] = {296: "A", 316: "B", 344: "~", 368: "C"}
HEARTBEAT_FOR_FORMAT: dict[str, bytes] = {
    "A": b"A", "B": b"B", "~": b"~", "C": b"C",
}


def packet_format_for(size: int) -> str | None:
    """The format letter for a raw packet length, or None if unrecognised."""
    return PACKET_FORMATS.get(size)
...
# Absolute byte offset of the steering / wheel-rotation float inside the
# extended packet, or None while unverified.
#
# The 72 bytes after 0x128 are undocumented by Polyphony.  Community sources
# put wheel rotation, filtered throttle/brake and body sway/heave/surge in
# there, but the offsets differ between sources, so this project measures it
...
class GT7Packet:
    ...
    car_id: int
    # Everything after byte 296, or None on the 'A' format.  Kept raw because
    # the layout in here is not yet verified against a live capture — see
    # STEERING_TAIL_OFFSET.
    tail: bytes | None = None
    # 'A' | 'B' | '~' | 'C', or None when the length is unrecognised.  The
    # export must declare this, so it travels with the packet rather than
    # being re-derived later.
    packet_format: str | None = None

    # ---------------------------------------------------------------- computed

    def tail_float(self, offset: int) -> float | None:
        """Read a little-endian float from the extended tail by absolute offset.

        Returns None when this packet had no tail, or when the value is not a
        finite float (which is how a wrongly-guessed offset shows up).
        """
        if self.tail is None:
            return None
        rel = offset - PACKET_SIZE
        if rel < 0 or rel + 4 > len(self.tail):
            return None
        value = struct.unpack_from('<f', self.tail, rel)[0]
    ...
    def surface_types(self) -> tuple[str, str, str, str] | None:
        ...
        if SURFACE_TAIL_OFFSET is None or self.tail is None:
            return None
        rel = SURFACE_TAIL_OFFSET - PACKET_SIZE
        if rel < 0 or rel + 4 > len(self.tail):
            return None
    ...
    @property
    def current_lap_time_ms(self) -> int | None:
        """Time on the lap in progress.  'C' format only; None elsewhere.

        Live strategy needs this — without it, progress through a lap can only
        be inferred from road distance.
        """
        if CURRENT_LAP_TAIL_OFFSET is None or self.tail is None:
            return None
        rel = CURRENT_LAP_TAIL_OFFSET - PACKET_SIZE
        if rel < 0 or rel + 4 > len(self.tail):
            return None
        value = struct.unpack_from('<i', self.tail, rel)[0]
        return value if value >= 0 else None

    @property
    def has_extended_tail(self) -> bool:
...
def parse_packet(data: bytes) -> GT7Packet | None:
    ...
    format introduced in GT7 v1.3+.  The first 296 bytes carry every field
    with a verified offset; the extended tail is kept raw on `.tail` so it can
    be decoded once its layout is confirmed against a live capture.
    """
    ...
    tail = data[PACKET_SIZE:] if len(data) > PACKET_SIZE else None
    return GT7Packet(*fields, tail=tail, packet_format=packet_format_for(len(data)))
